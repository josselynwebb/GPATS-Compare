...
Module DscopeMain
    ...
    Public bIsHPE1428A As Boolean = False
    Public hHPE1428A As Integer
    ...
    Public Const CHANNEL_1 As Short = 0
    Public Const CHANNEL_2 As Short = 1
    Public Const MATH_AREA As Short = 2
    Public Const MEMORY_AREA As Short = 3
    ...
    Dim MEMRangeSetting As Single = 1
    ...
    Public Const RESOURCE_NAME As String = "VXI::17"
    ...
    Sub SetMathRange(ByRef TraceIndex As Short, ByRef InstrumentRange As Short)
        ...
        frmZT1428.cwgScopeDisplay.YAxes(MATH_AREA).Range = New NationalInstruments.UI.Range((-VRAnge * 5.0!) + Voffset, (VRAnge * 5.0!) + Voffset)
    ...
    Sub GetMemoryWave(ByRef WvMagn(,) As Double, ByRef TraceNumber As Short, ByRef ChangeRange As Short)
        ...
        Try
              frmZT1428.cwgScopeDisplay.YAxes(MEMORY_AREA).Range = New NationalInstruments.UI.Range((-MEMRangeSetting * 5) + MEMOffsetSetting, (MEMRangeSetting * 5) + MEMOffsetSetting)
    ...
    Sub AutoScaleScope()
        'Declare Procedure Level variables
        Dim RangeCh1 As Double
        Dim RangeCh2 As Double
        ...
        frmZT1428._chkDisplayTrace_0.Enabled = False
        frmZT1428._chkDisplayTrace_1.Enabled = False
        frmZT1428._chkDisplayTrace_2.Enabled = False
        frmZT1428._chkDisplayTrace_3.Enabled = False
        ...
        For RangeIndex = 0 To 11
            If Val(ParamCode(IN1_VSENS, RangeIndex)) >= RangeCh1 Then
                If Val(ParamCode(IN1_VSENS, RangeIndex + 1)) < RangeCh1 Then
                    frmZT1428._cboVertRange_0.SelectedIndex = RangeIndex
                    WriteMsg("CHAN1:RANG " & ParamCode(IN1_VSENS, RangeIndex))
                ElseIf RangeIndex = 11 Then
                    frmZT1428._cboVertRange_0.SelectedIndex = 0
            ...
            If Val(ParamCode(IN2_VSENS, RangeIndex)) >= RangeCh2 Then
                If Val(ParamCode(IN2_VSENS, RangeIndex + 1)) < RangeCh2 Then
                    frmZT1428._cboVertRange_1.SelectedIndex = RangeIndex
                    WriteMsg("CHAN2:RANG " & ParamCode(IN2_VSENS, RangeIndex))
                ElseIf RangeIndex = 11 Then
                    frmZT1428._cboVertRange_1.SelectedIndex = 0
        ...
        If Trim(HoldElem(1)) = "TIME" Then
            HoldTime = HoldElem(2)
            frmZT1428._optHoldOff_0.Checked = True
        ...
        If Trim(StripNullCharacters(TrigSlope)) = ParamCode(TRIG_SLOPE, 0) Then
            frmZT1428._optTrigSlope_0.Checked = True
        Else
            frmZT1428._optTrigSlope_1.Checked = True
        End If
        ...
        For Source = 0 To 4
            ...
            End If
        Next Source
        frmZT1428._optReference_0.Checked = True
        frmZT1428._optTrigMode_0.Checked = True
        FillTextBoxes()
        ...
        If Not ContinuousFlag Then
            If bTIP_Running <> True Then
                ...
                'enabled channel 1 and 2
                ChannelOn(CHANNEL_1) = True
                frmZT1428.cwgScopeDisplay.Plots(CHANNEL_2).Visible = True
                SelectTraces(CHANNEL_1, True)
                ChannelOn(CHANNEL_2) = True
                frmZT1428.cwgScopeDisplay.Plots(CHANNEL_2).Visible = True
                SelectTraces(CHANNEL_2, True)
                frmZT1428._chkDisplayTrace_0.Checked = True
                frmZT1428._chkDisplayTrace_1.Checked = True
                'Sample Waveform
                DigitizeWaveform()
            End If
        Else
        ...
        If InStr(Coupling, "AC") Then
            frmZT1428._optCoupling1_2.Checked = True
        End If
        WriteMsg("CHAN2:COUP?")
        Coupling = StripNullCharacters(ReadMsg())
        If InStr(Coupling, "AC") Then
            frmZT1428._optCoupling2_2.Checked = True
        End If
    ...
    Sub ChangeMeasurement(ByRef MeasIndex As Short)
        ...
        For i As Integer = 1 To frmZT1428.tbrMeasFunction.Buttons.Count - 1
        Dim newToolbarButton As ToolBarButton = frmZT1428.tbrMeasFunction.Buttons.Item(i)
            If (i = MeasIndex) Then
                newToolbarButton.Pushed = True
            Else
                newToolbarButton.Pushed = False
            End If
    ...
    Sub ChangeTraceColor()
        ...
        If WhatTrace(TraceToChange) <> -1 Then
            frmZT1428.cwgScopeDisplay.Plots(WhatTrace(TraceToChange)).PointColor = System.Drawing.ColorTranslator.FromOle(TraceColor(TraceToChange))
        End If
    ...
    Sub ChangeTraceThickness()
        'Declare Procedure Level variables
        Dim newPointSize As System.Drawing.Size

        Select Case frmZT1428.cboTraceThickness.SelectedIndex
            Case 0
                'LineStyle = PointStyle.SolidCircle
                newPointSize = New Drawing.Size(1, 1)
            Case 1
                'LineStyle = PointStyle.SolidDiamond
                newPointSize = New Drawing.Size(2, 2)
            Case 2
                'LineStyle = PointStyle.SolidSquare
                newPointSize = New Drawing.Size(3, 3)
        End Select
        frmZT1428.cwgScopeDisplay.Plots(0).PointSize = newPointSize
        frmZT1428.cwgScopeDisplay.Plots(1).PointSize = newPointSize
        frmZT1428.cwgScopeDisplay.Plots(2).PointSize = newPointSize
        frmZT1428.cwgScopeDisplay.Plots(3).PointSize = newPointSize
    ...
    Sub DigitizeWaveform()
        ...
        If InStr(1, sTIP_Mode, "TIP_RUN") > 0 Or InStr(1, sTIP_Mode, "TIP_MEAS") > 0 Then
            ...
            End If
            If sTIP_Mode = "TIP_RUNPERSIST" Then frmZT1428.chkContinuous.Checked = True
        ...
        Do
            ...
            If ChannelOn(CHANNEL_1) Then
                ...
                System.Windows.Forms.Application.DoEvents()
                ReDim Ch1TempWave(WvPoints)
                ReDim WaveTime(WvPoints)
                GetWaveform(Ch1TempWave, WaveTime, dYIncr, dYOrig, dXIncr, dXOrig, CHANNEL_1, NumberOfTraces, AcqMode)
                System.Windows.Forms.Application.DoEvents()

                frmZT1428.cwgScopeDisplay.Plots(CHANNEL_1).PlotXY(WaveTime, Ch1TempWave)
            ...
            If ChannelOn(CHANNEL_2) Then
                ...
                System.Windows.Forms.Application.DoEvents()
                ReDim Ch2TempWave(WvPoints)
                ReDim WaveTime(WvPoints)
                GetWaveform(Ch2TempWave, WaveTime, dYIncr, dYOrig, dXIncr, dXOrig, CHANNEL_2, NumberOfTraces, AcqMode)
                System.Windows.Forms.Application.DoEvents()

                frmZT1428.cwgScopeDisplay.Plots(CHANNEL_2).PlotXY(WaveTime, Ch2TempWave)
            ...
            If ChannelOn(MATH_AREA) Then
                ...
                System.Windows.Forms.Application.DoEvents()
                If (bIsHPE1428A) Then 'HPE1428A
                    dYIncr = Val(MainParam(FUNC_RANGE).SetCur) / 65536
                    dYOrig = -(Val(MainParam(FUNC_RANGE).SetCur) / 2) + Val(MainParam(IN2_OFFSET).SetCur)
                    GetWaveform(MathTempWave, WaveTime, dYIncr, dYOrig, dXIncr, dXOrig, MATH_AREA, NumberOfTraces, AcqMode)
                Else 'ZT1428
                    GetWaveform(MathTempWave, WaveTime, dYIncr, dYOrig, dXIncr, dXOrig, MATH_FUNC1, NumberOfTraces, AcqMode)
                End If

                System.Windows.Forms.Application.DoEvents()

                frmZT1428.cwgScopeDisplay.Plots(MATH_AREA).PlotXY(WaveTime, MathTempWave)
            ...
            If ChannelOn(MEMORY_AREA) Then
                System.Windows.Forms.Application.DoEvents()
                ReDim MemTempWave(WvPoints)
                ReDim WaveTime(WvPoints)
                GetWaveform(MemTempWave, WaveTime, dYIncr, dYOrig, dXIncr, dXOrig, MEM_WMEM1, NumberOfTraces, AcqMode)
                System.Windows.Forms.Application.DoEvents()
  
                frmZT1428.cwgScopeDisplay.Plots(MEMORY_AREA).PlotXY(WaveTime, MemTempWave)
            ...
            frmZT1428.cwgScopeDisplay.Cursors(0).XPosition = 0
            frmZT1428.cwgScopeDisplay.Cursors(0).YPosition = 0
            UpdateCursorCaption(System.Math.Abs(frmZT1428.cwgScopeDisplay.Cursors(0).XPosition), 0)
        ...
        Loop Until (frmZT1428.chkContinuous.Checked = False)
        ...
        frmZT1428._chkDisplayTrace_0.Enabled = True
        frmZT1428._chkDisplayTrace_1.Enabled = True
        If Val(MainParam(WF_POINTS).SetCur) <> 8000 Then
            frmZT1428._chkDisplayTrace_2.Enabled = True
        End If
        If frmZT1428.cboSampleType.SelectedIndex = REALTIME Then
            frmZT1428._optDataPoints_0.Enabled = True
            frmZT1428._optDataPoints_1.Enabled = True
        ...
        frmZT1428._chkDisplayTrace_3.Enabled = True
    ...
    Sub FillVSense(ByRef Chan As Short, ByRef AttIndex As Short)
        ...
        Dim DummyParam As New InstParameter
        ...
        If (Chan = CHANNEL_1) Then
            tempComboBox = frmZT1428._cboVertRange_0
        ElseIf (Chan = CHANNEL_2) Then
            tempComboBox = frmZT1428._cboVertRange_1
        End If
        ...
        If (tempComboBox.Name IsNot Nothing) Then
            ...
            tempComboBox.Items.Insert(RangeSetting, NewItem)
        Next RangeSetting
        tempComboBox.SelectedIndex = VTraceListItem
        End If
    ...
    Sub LoadDiskToMemory()
        ...
        For PreambleIndex = 1 To Microsoft.VisualBasic.AscW(AsciiChar)
        ...
        '------------------------------------------------------
        frmZT1428.chkContinuous.Checked = False 'Stop All Data Acquisition
        ...
        If Points = 8000 Then
            frmZT1428._optDataPoints_1.Checked = True
        Else
            frmZT1428._optDataPoints_0.Checked = True
        End If
...
TIP_ErrorHandle:
    ...
    Sub RegisterChange(ByRef ParamIndex As Short)
        ...
        Select Case ParamIndex
            ...
            Case FUNC_RANGE, FUNC_OFFSET
                ...
                Voffset = Val(MainParam(FUNC_OFFSET).SetCur)
                frmZT1428.cwgScopeDisplay.YAxes(MATH_AREA).Range = New NationalInstruments.UI.Range((-VRAnge / 2.0!) + Voffset, (VRAnge / 2.0!) + Voffset)
                Exit Sub
            Case MEM_RANGE, MEM_OFFSET
                VRAnge = Val(MainParam(MEM_RANGE).SetCur)
                Voffset = Val(MainParam(MEM_OFFSET).SetCur)
                frmZT1428.cwgScopeDisplay.YAxes(MEMORY_AREA).Range = New NationalInstruments.UI.Range((-VRAnge * 5) + Voffset, (VRAnge * 5) + Voffset)
        End Select
    ...
    Sub ResetScope()
        ...
        'mh SoftReset
        frmZT1428.chkContinuous.Checked = False
        ...
        frmZT1428.tbrMeasFunction.Buttons(20).Pushed = 1
        frmZT1428._optStartSlope_0.Checked = True
        frmZT1428._optStartLevel_0.Checked = True
        frmZT1428._optStopSlope_0.Checked = True
        frmZT1428._optReference_0.Checked = True
        frmZT1428._optHoldOff_0.Checked = True
        frmZT1428._optDataPoints_0.Checked = True
        frmZT1428._optProbe1_0.Checked = True
        frmZT1428._optCoupling1_0.Checked = True
        frmZT1428._optProbe2_0.Checked = True
        frmZT1428._optCoupling2_0.Checked = True
        frmZT1428._optTrigSlope_0.Checked = True
        frmZT1428._optNoiseRej_0.Checked = True
        frmZT1428._optTrigMode_0.Checked = True
        frmZT1428._optTrigDelay_0.Checked = True
        frmZT1428._optDelaySlope_0.Checked = True
        frmZT1428._optStandard_0.Checked = True
        frmZT1428._optField_0.Checked = True
        frmZT1428._optExtTrigCoup_0.Checked = True

        frmZT1428._chkDisplayTrace_0.Checked = True
        frmZT1428._chkDisplayTrace_1.Checked = False
        frmZT1428._chkDisplayTrace_2.Checked = False
        frmZT1428._chkDisplayTrace_3.Checked = False

        frmZT1428.chkHFReject1.Checked = False
        frmZT1428.chkLFReject1.Checked = False
        frmZT1428.chkHFReject2.Checked = False
        frmZT1428.chkLFReject2.Checked = False

        frmZT1428.chkLFReject1.Enabled = False 'JHill TDR99151
        frmZT1428.chkLFReject2.Enabled = False 'JHill TDR99151

        frmZT1428.chkExternalOutput.Checked = False
        frmZT1428.chkECL0.Checked = False
        frmZT1428.chkECL1.Checked = False

        frmZT1428.cboMeasSour.Visible = True
        frmZT1428.fraDelayParam.Visible = False

        frmZT1428.txtDataDisplay.Text = "Ready..."
        frmZT1428.ribCompOFF.Checked = True
        frmZT1428.imgProbeComp.Visible = False
        frmZT1428.cwgScopeDisplay.ClearData()

        For Each ToolBarButton As ToolBarButton In frmZT1428.tbrMeasFunction.Buttons
            ToolBarButton.Pushed = False
        Next

        frmZT1428.tbrMeasFunction.Buttons(20).Pushed = True
    ...
    Sub SaveToMemory(ByRef Index As Short)
        ...
        If ChannelOn(MEMORY_AREA) Then
            ...
            frmZT1428.cwgScopeDisplay.Plots(MEMORY_AREA).PlotXY(WaveTime(0), WaveMagnitude(0, 0))
        End If
        WriteMsg("RUN")
        ...
        Select Case ParamIndex
            ...
            Case TRIG_MODE
                Select Case SelectIndex
                    ...
                    Case DEL_TRIG
                        ...
                        If frmZT1428._optTrigDelay_1.Checked = True Then
                            frmZT1428.fraTrigDelaySource.Visible = True
                            frmZT1428.fraDelaySlope.Visible = True
                        End If
                        frmZT1428.fraStandard.Visible = False
            ...
            Case WF_POINTS
                If SelectIndex = 1 Then
                    ...
                    frmZT1428.txtDataDisplay.Text = "Ready..."
                Else
                    If frmZT1428.chkContinuous.Checked = False Then
                        frmZT1428._chkDisplayTrace_2.Enabled = True
                    End If
        ...
        Select Case ParamIndex
            ...
            Case SAMPLE_MODE
                ...
                Else
                    ...
                    frmZT1428._optDataPoints_0.Enabled = False
                    frmZT1428._optDataPoints_1.Enabled = False
            ...
            Case ACQ_MODE
                ...
                End If
            Case IN1_COUP
                If (ParamCode(ParamIndex, SelectIndex) = "AC") Then
                    frmZT1428.chkLFReject1.Enabled = True
                Else
                    frmZT1428.chkLFReject1.Checked = False
                    frmZT1428.chkLFReject1.Enabled = False
                End If
            Case IN2_COUP
                If (ParamCode(ParamIndex, SelectIndex) = "AC") Then
                    frmZT1428.chkLFReject2.Enabled = True
                Else
                    frmZT1428.chkLFReject2.Checked = False
                    frmZT1428.chkLFReject2.Enabled = False
                End If
        End Select
    ...
    Sub SelectTraces(ByRef TraceDisplay As Short, ByRef Value As Short)
        ...
        Dim TempWaveMagnitude(2000, 2000) As Double
        Dim Ch1TempWave(,) As Double
        Dim Ch2TempWave(,) As Double
        Dim MathTempWave(,) As Double
        Dim MemTempWave(,) As Double
        ...
        ReDim WaveMagnitude(3, DataPoints)
        ...
        For ChannelNumber = 0 To 3
            If ChannelOn(ChannelNumber) Then
                ...
                frmZT1428.cwgScopeDisplay.Plots.Item(ChannelNumber).PointColor = System.Drawing.ColorTranslator.FromOle((TraceColor(ChannelNumber)))
            Else
            ...
            If TraceAssociation(TraceOn) <> MEMORY_AREA Then
                ...
                Select Case TraceAssociation(TraceOn)
                    Case CHANNEL_1
                        ...
                        frmZT1428.cwgScopeDisplay.Plots(CHANNEL_1).PointColor = System.Drawing.ColorTranslator.FromOle(TraceColor(CHANNEL_1))
                    ...
                    Case CHANNEL_2
                        ...
                        frmZT1428.cwgScopeDisplay.Plots(CHANNEL_2).PointColor = System.Drawing.ColorTranslator.FromOle(TraceColor(CHANNEL_2))
                        For i = 0 To TempWvPts
                    ...
                    Case MATH_AREA
                        ...
                        Voffset = Val(MainParam(FUNC_OFFSET).SetCur)
                        frmZT1428.cwgScopeDisplay.Plots(MATH_AREA).PointColor = System.Drawing.ColorTranslator.FromOle(TraceColor(MATH_AREA))
                        For i = 0 To TempWvPts
                            MathTempWave(0, i) = WaveMagnitude(TraceOn, i)
                        Next i
                End Select
                frmZT1428.cwgScopeDisplay.YAxes(TraceOn).Range = New NationalInstruments.UI.Range((-Vscale / 2.0!) + Voffset, (Vscale / 2.0!) + Voffset)
            Else
    ...
    Sub SetHScale()
        ...
        frmZT1428.StatusBar1.Items(HORZ_SEN_PAN).Text = "Hor. Axis = " & frmZT1428.cboHorizRange.Text
        SetHScaleGraph()
    ...
    Sub SetHScaleGraph()
        ...
        Select Case MainParam(H_REF_POS).SetCur
            Case H_LEFT
                frmZT1428.cwgScopeDisplay.XAxes(0).Range = New NationalInstruments.UI.Range(DDelay, HScale * PointMlt + DDelay)
            Case H_CENTER
                frmZT1428.cwgScopeDisplay.XAxes(0).Range = New NationalInstruments.UI.Range(-(HScale * PointMlt / 2) + DDelay, (HScale * PointMlt / 2) + DDelay)
                TempAxis = System.Math.Abs(frmZT1428.cwgScopeDisplay.XAxes(0).Range.Minimum)
            Case H_RIGHT
                frmZT1428.cwgScopeDisplay.XAxes(0).Range = New NationalInstruments.UI.Range(-HScale * PointMlt + DDelay, DDelay)
        End Select

        'Set Cursors to Center
        frmZT1428.cwgScopeDisplay.Cursors(0).XPosition = 0
        frmZT1428.cwgScopeDisplay.Cursors(0).YPosition = 0
    ...
    Sub SetVScaleGraph(ByRef VertIndex As Short)
        ...
        If VertIndex = CHANNEL_1 Then
            MainParam(IN1_VSENS).SetCur = ParamCode(IN1_VSENS, frmZT1428._cboVertRange_0.SelectedIndex)
        ...
        If VertIndex = CHANNEL_2 Then
            MainParam(IN2_VSENS).SetCur = ParamCode(IN2_VSENS, frmZT1428._cboVertRange_1.SelectedIndex)
        ...
        End If
        If frmZT1428.chkContinuous.Checked = True Then
            frmZT1428.cwgScopeDisplay.ClearData()
        End If
        'Filter out invalid WhatTrace selections so that XAxis does not accidently get set
        If WhatTrace(VertIndex) >= 0 Then
            frmZT1428.cwgScopeDisplay.YAxes(VertIndex).Range = New NationalInstruments.UI.Range((-Vscale / 2.0!) + offset, (Vscale / 2.0!) + offset)
        End If
    ...
    Sub SetTrigRange(ByRef ChanIndex As Short)
        ...
        If ChanIndex = frmZT1428.cboTrigSour.SelectedIndex Then
            ...
            MainParam(TRIG_LEV).TxtBox.Text = EngNotate(Val(MainParam(TRIG_LEV).SetCur), MainParam(TRIG_LEV))
    ...
    Public Sub Main()
        ...
        Dim IDNString As String = Space(256)

        'Determine Scope Type
        lpFileName = Registry.GetValue("HKEY_LOCAL_MACHINE\SOFTWARE\ATS\", "IniFilePath", Nothing)
        GetPrivateProfileString("OSCOPE", "IDN", "", IDNString, 256, lpFileName)
        If IDNString.Contains("E1428A") Then
            bIsHPE1428A = True
        Else
            bIsHPE1428A = False
        End If
        ...
        sWindowsDir = System.Environment.GetEnvironmentVariable("windir")
        ...
        frmZT1428.StatusBar1.Items(HORZ_SEN_PAN).Text = "X Axis: " & frmZT1428.cboHorizRange.Text
        frmZT1428.StatusBar1.Items(CURS_POS_PAN).Text = "Cursor Position = 0,0"
        frmZT1428.tabMainFunctions.SelectedIndex = 0
        ...
        If bTIP_Running Then
            ...
            'Make sure scope is in local mode
            ...
            Select Case sTIP_Mode
                Case "TIP_RUNPERSIST", "TIP_MANUAL"
                    frmZT1428.chkContinuous.Checked = True
                    DigitizeWaveform()
                Case "TIP_DESIGN"
        ...
        'Write Ready Message to Status Bar
        frmZT1428.txtDataDisplay.Text = "Ready"
    ...
    Sub UpdateCursorCaption(ByRef XPos As Double, ByRef YPos As Double)
        ...
        My.Forms.frmZT1428.StatusBar1.Items(CURS_POS_PAN).Text = "Cursor Position = " & XAxisPos & "," & YAxisPos
        If bTIP_Running Then
    ...
    Sub StatusBarMessage(ByRef MessageString As String)
        ...
        If frmZT1428.StatusBar1.Items(3).Text <> MessageString Then
            frmZT1428.StatusBar1.Items(3).Text = MessageString
        End If
    ...
    Private Sub ConfigureFrontPanel()
        ...
        frmZT1428._chkDisplayTrace_0.Checked = False
        frmZT1428._chkDisplayTrace_1.Checked = False
        frmZT1428._chkDisplayTrace_2.Checked = False
        frmZT1428._chkDisplayTrace_3.Checked = False
        ...
        FillTextBoxes()
        If frmZT1428._chkDisplayTrace_2.Checked = True Then
...
ExitPoint:
            ...
            With frmZT1428
                ...
                Select Case i
                    ...
                    Case IN1_COUP '.SetCod = ":CHAN1:COUP "
                        If Not ._chkDisplayTrace_0.Checked Then ._chkDisplayTrace_0.Checked = True 'Enable CH1 tab
                        Select Case sArg
                            Case "DC" : ._optCoupling1_0.Checked = True
                            Case "DCF" : ._optCoupling1_1.Checked = True
                            Case "AC" : ._optCoupling1_2.Checked = True
                    ...
                    Case IN1_HFREJ '.SetCod = ":CHAN1:HFR "
                        If Not ._chkDisplayTrace_0.Checked Then ._chkDisplayTrace_0.Checked = True 'Enable CH1 tab
                        Select Case sArg
                            Case "OFF" : .chkHFReject1.Checked = False
                            Case "ON" : .chkHFReject1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case IN1_LFREJ '.SetCod = ":CHAN1:LFR "
                        If Not ._chkDisplayTrace_0.Checked Then ._chkDisplayTrace_0.Checked = True 'Enable CH1 tab
                        Select Case sArg
                            Case "OFF" : .chkLFReject1.Checked = False
                            Case "ON" : .chkLFReject1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case IN1_ATTN_FAC '.SetCod = ":CHAN1:PROB "
                        If Not ._chkDisplayTrace_0.Checked Then ._chkDisplayTrace_0.Checked = True 'Enable CH1 tab
                        Select Case sArg
                            Case "1.00000E+00" : ._optProbe1_0.Checked = True 'Default
                            Case "1.00000E+01" : ._optProbe1_1.Checked = True
                            Case "1.00000E+02" : ._optProbe1_2.Checked = True
                            Case "1.00000E+03" : ._optProbe1_3.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case IN2_COUP '.SetCod = ":CHAN2:COUP "
                        If Not ._chkDisplayTrace_1.Checked Then ._chkDisplayTrace_1.Checked = True 'Enable CH2 tab
                        Select Case sArg
                            Case "DC" : ._optCoupling2_0.Checked = True
                            Case "DCF" : ._optCoupling2_1.Checked = True
                            Case "AC" : ._optCoupling2_2.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case IN2_HFREJ '.SetCod = ":CHAN2:HFR "
                        If Not ._chkDisplayTrace_1.Checked Then ._chkDisplayTrace_1.Checked = True 'Enable CH2 tab
                        Select Case sArg
                            Case "OFF" : .chkHFReject2.Checked = False
                            Case "ON" : .chkHFReject2.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case IN2_LFREJ '.SetCod = ":CHAN2:LFR "
                        If Not ._chkDisplayTrace_1.Checked Then ._chkDisplayTrace_1.Checked = True 'Enable CH2 tab
                        Select Case sArg
                            Case "OFF" : .chkLFReject2.Checked = False
                            Case "ON" : .chkLFReject2.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case IN2_ATTN_FAC '.SetCod = ":CHAN2:PROB "
                        If Not ._chkDisplayTrace_1.Checked Then ._chkDisplayTrace_1.Checked = True 'Enable CH2 tab
                        Select Case sArg
                            Case "1.00000E+00" : ._optProbe2_0.Checked = True 'Default
                            Case "1.00000E+01" : ._optProbe2_1.Checked = True
                            Case "1.00000E+02" : ._optProbe2_2.Checked = True
                            Case "1.00000E+03" : ._optProbe2_3.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                        'Reset Vertical range combo box values based upon ATTEN setting
                        If sErrMsg = "" And Val(sArg) > 2 Then
                    ...
                    Case ECLT0_STATE '.SetCod = ":OUTP:ECLT0:STATE "
                        Select Case sArg
                            Case "OFF" : .chkECL0.Checked = False
                            Case "ON" : .chkECL0.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select

                    Case ECLT1_STATE '.SetCod = ":OUTP:ECLT1:STATE "
                        Select Case sArg
                            Case "OFF" : .chkECL1.Checked = False
                            Case "ON" : .chkECL1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select

                    Case OUT_TRIG_STATE '.SetCod = ":OUTP:EXT:STATE "
                        Select Case sArg
                            Case "OFF" : .chkExternalOutput.Checked = False
                            Case "ON" : .chkExternalOutput.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case TRIG_MODE '.SetCod = ":TRIG:MODE "
                        Select Case sArg
                            Case "EDGE" : ._optTrigMode_0.Checked = True
                            Case "DEL" : ._optTrigMode_1.Checked = True
                            Case "TV" : ._optTrigMode_2.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case TRIG_SLOPE '.SetCod = ":TRIG:SLOP "
                        Select Case sArg
                            Case "POS" : ._optTrigSlope_0.Checked = True
                            Case "NEG" : ._optTrigSlope_1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select

                    Case TRIG_SENS '.SetCod = ":TRIG:SENS "
                        Select Case sArg
                            Case "NORM" : ._optNoiseRej_0.Checked = True
                            Case "LOW" : ._optNoiseRej_1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case TRIG_DEL '.SetCod = ":TRIG:DEL "
                        ...
                        If sErrMsg = "" Then
                            Select Case sArgList(1)
                                Case "TIME" : ._optTrigDelay_0.Checked = True
                                Case "EVEN" : ._optTrigDelay_1.Checked = True
                                Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                            End Select
                            Delay(0.5)
                    ...
                    Case TRIG_DEL_SLOPE '.SetCod = ":TRIG:DEL:SLOP "
                        Select Case sArg
                            Case "POS" : ._optDelaySlope_0.Checked = True
                            Case "NEG" : ._optDelaySlope_1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case TRIG_STAND '.SetCod = ":TRIG:STAN "
                        Select Case sArg
                            Case "525" : ._optStandard_0.Checked = True
                            Case "625" : ._optStandard_1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select

                    Case TRIG_FIELD '.SetCod = ":TRIG:FIEL "
                        Select Case sArg
                            Case "1" : ._optField_0.Checked = True
                            Case "2" : ._optField_1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case EXT_TRIG_COU '.SetCod = ":TRIG:COUP "
                        Select Case sArg
                            Case "DC" : ._optExtTrigCoup_0.Checked = True
                            Case "DCF" : ._optExtTrigCoup_1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case HOLD_OFF '.SetCod = ":TRIG:HOLD "
                        ...
                        If sErrMsg = "" Then
                            Select Case sArgList(1)
                                Case "TIME" : ._optHoldOff_0.Checked = True
                                Case "EVEN" : ._optHoldOff_1.Checked = True
                                Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                            End Select
                            Delay(0.5)
                    ...
                    Case WF_POINTS '.SetCod = ":ACQ:POINTS "
                        Select Case sArg
                            Case "500" : ._optDataPoints_0.Checked = True
                            Case "8000" : ._optDataPoints_1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case H_REF_POS '.SetCod = ":TIM:REF "
                        Select Case sArg
                            Case "CENT" : ._optReference_0.Checked = True
                            Case "LEFT" : ._optReference_1.Checked = True
                            Case "RIGH" : ._optReference_2.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                    ...
                    Case MEAS_UNIT
                        ...
                        Select Case sArg
                            Case "VOLT" : ._optStartLevel_0.Checked = True
                            Case "PERC" : ._optStartLevel_1.Checked = True
                            Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                        End Select
                        'End If
                    ...
                    Case MEAS_DEF
                        ...
                        If sErrMsg = "" Then
                            Select Case sArgList(2)
                                Case "POS" : ._optStartSlope_0.Checked = True
                                Case "NEG" : ._optStartSlope_1.Checked = True
                                Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                            End Select
                            ...
                            Select Case sArgList(5)
                                Case "POS" : ._optStopSlope_0.Checked = True
                                Case "NEG" : ._optStopSlope_1.Checked = True
                                Case Else : sErrMsg = "Invalid " & sCOMPcmd & " argument passed from TIP Studio."
                            End Select
                    ...
                    Case FUNC_RANGE 'GUI only
                        ._chkDisplayTrace_2.Checked = True 'Enable Math area of Math/Memory tab
                    ...
                    Case SEGMENT_FILE
                        frmZT1428._chkDisplayTrace_3.Checked = True
                        sArg = UCase(sArg)
                        If sArg <> "" And InStr(2, sArg, ".SEG") > 1 Then
                            sSegFileName = sArg
                            LoadDiskToMemory()
                            ._chkDisplayTrace_3.Checked = True
                        Else
                    ...
                    Case MEM_RANGE 'GUI only
                        ._chkDisplayTrace_3.Checked = True 'Enable Math area of Math/Memory tab
    ...
    Public Function bSaveDSO_File(ByRef sCmds As String) As Boolean
        ...
        With frmZT1428
            ...
            'TIR 3.0.0.0010-009 NAM File Export does not work
            'Setup File I/O dialog
            .dlgFileIOOpen.DefaultExt = "DSO"
            .dlgFileIOOpen.Filter = "DSO File (*.DSO)|*.DSO|All Files (*.*)|*.*"
            If .dlgFileIOOpen.FileName = "" Then
                .dlgFileIOOpen.FileName = "Test1.DSO"
            End If

            .dlgFileIOSave.ShowDialog()
            .dlgFileIOOpen.FileName = .dlgFileIOSave.FileName

            'Confirm File Replace
            If FileExists(.dlgFileIOOpen.FileName) Then
                If MsgBox("This folder already contains a file named '" & _
                    .dlgFileIOOpen.FileName & vbCrLf & _
                    "Would you like to replace it?", _
                    vbYesNo + vbQuestion, "Confirm File Replace") = vbYes Then
                    Kill(.dlgFileIOOpen.FileName)
                Else
                    Exit Function
                End If
            End If
            ...
            iFileId = FreeFile()
            FileOpen(iFileId, .dlgFileIOOpen.FileName, OpenMode.Output)
            'Write the file
...
FileErrHandler:
    ...
    Sub WaitForTrigger()
        ...
        ElseIf frmZT1428.txtDataDisplay.Text = "Waiting for Trigger" Then
            frmZT1428.txtDataDisplay.Text = ""
        End If
        If Not frmZT1428.chkContinuous.Checked Then
