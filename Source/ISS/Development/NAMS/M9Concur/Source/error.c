...
char	*error_message[] = {
	...
	"The dtb file was not listed",									/*NO_DTB_STATED*/
	...
	"Diagnostics is not allowed in concurrent mode yet",			/*DIAG_NOT_ALLOWED*/
 ...
 *
 */
int error_ms(int code, char *function_name, char *string, int do_return_status)
{
	char	tmpcharstring[1024];

	if (debugfp == NULL && do_return_status == FALSE) 
	{
	...
	sprintf(tmpcharstring, "%s in %s\r\n", code == 0 ? string : code < 700 ? strerror(code) : error_message[ code == 999 ? code - 954 : code < 800 ? code - 700 : code - 792 ], function_name);
	...
	sprintf(pipe_info.write_msg, "%s%s", ERROR, tmpcharstring);
	return(SUCCESS);
}
...
void dodebug(int code, char *function_name, char *string)
{
	if (DE_BUG)
	{
		if (error_ms(code, function_name, string, FALSE)) {
	...
	sprintf(err_info.func_name, "%s", function_name);
	sprintf(err_info.error_string, "%s", string);
}

/*
 *	return_status:	This function will return th status of the m9concur program
...
void return_status()
{
	/*Look at the dtb_info.dtb_errno to see if it is ! BURST_PASSED.  If it is not
	BURST_PASSED then something bad (ERROR) happen.  This we will send back as best as
	as we can to the m910nam program.  If the m910nam program can receive a proper
	response from us it will terminate us.*/
	if (dtb_info.dtb_errno != BURST_PASSED && dtb_info.dtb_errno != BURST_FAILED && dtb_info.dtb_errno != BURST_NOT_RUN)
	{
		error_ms(err_info.error_code, err_info.func_name, err_info.error_string, TRUE);
		write_message();
	}

	else 
	{
		switch(dtb_info.dtb_errno)
		{
			case BURST_PASSED:

				sprintf(pipe_info.write_msg, "%s", "SUCCESS");
				pipe_info.write_msg[strlen("SUCCESS")] = '\0';

				break;

			case BURST_FAILED:

				sprintf(pipe_info.write_msg, "%s", "FAILED");
				pipe_info.write_msg[strlen("FAILED")] = '\0';

				break;

			case BURST_NOT_RUN:

				sprintf(pipe_info.write_msg, "%s", "BURST NOT RUN");
				pipe_info.write_msg[strlen("BURST NOT RUN")] = '\0';

				break;

			default:

				sprintf(pipe_info.write_msg, "%s", "ERROR Something unknown happened\r\n");
				pipe_info.write_msg[strlen("ERROR Something unknown happened\r\n")] = '\0';
