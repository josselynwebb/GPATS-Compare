...
static const char plugin_id[] = SOAP_DUMP_PLUGIN_ID;
static const char plugin_location[] = SOAP_DUMP_PLUGIN_LOCATION;
...
{ if (!(dst->data = (struct plugin_data*)malloc(sizeof(struct plugin_data))))
    return SOAP_EOM;		
  memcpy (dst->data, src->data, sizeof(struct plugin_data));//*dst->data = *src->data;
  return SOAP_OK;
}
...
void print_soap_envelope(const char* source, const char* direction, const char *buf, size_t len){
	time_t start;
	FILE *fp = NULL;
	char filename[512];
	memset((void *)filename, 0, 512);
	time (&start); //seconds since epoc
	sprintf((char *)filename, "c:/aps/data/soap_%ld_%s_%s.xml", (long) start, source, direction);
...
{ 
	...
	print_soap_envelope(plugin_location, "SEND", buf, len);
...
{ 
	...
	print_soap_envelope(plugin_location, "RECV", buf, res);
	return res;
}
