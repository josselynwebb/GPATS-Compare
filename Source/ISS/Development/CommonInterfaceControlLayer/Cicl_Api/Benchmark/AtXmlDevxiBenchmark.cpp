...
static void s_ParseValDataResponse(char *Response, int BufferSize);
static void s_ParseErrDbgResponse(char *Response, int BufferSize);
...
/**/
    // Issue TestStationStatus test
	printf("Call atxml_TestStationStatus(Setup)\n");
    // Create a single test requirement XML Snippet (not in interface yet)
    sprintf(XmlBuf,
        "<AtXmlTestRequirements> "
		"    <ResourceRequirement> "
		"	    <ResourceType>Source</ResourceType> "
    ...
    Status = atxml_TestStationStatus(Response, 4096);
    printf("atxml_TestStationStatus = %d\n  Response:\n",Status);
    s_ParseErrDbgResponse(Response, 4096);
    s_ParseValDataResponse(Response, 4096);
getchar();
/**/
...
/**/
    // Issue Commands Test
	printf("Call atxml_InvokeRemoveAllSequence()\n");
    Status = atxml_InvokeRemoveAllSequence(Response, 4096);
    printf("atxml_InvokeRemoveAllSequence = %d\n  Response:\n",Status);
    s_ParseErrDbgResponse(Response, 4096);
getchar();
/**/
...
/**/
    // Issue Self Test
	printf("Call atxml_RegisterRemoveSequence()\n");
    sprintf(XmlBuf,
	"<AtXmlRemoveSequence>"
	"   <RemoveAssignment sequenceNumber=\"0\" resourceName=\"DevX_1\"/>"
	"   <RemoveAssignment sequenceNumber=\"1\" resourceName=\"PAWS_SWITCH\"/>"
	"</AtXmlRemoveSequence>"
        );
    Status = atxml_RegisterRemoveSequence(XmlBuf, Response, 4096);
    printf("atxml_RegisterRemoveSequence = %d\n  Response:\n",Status);
    s_ParseErrDbgResponse(Response, 4096);
    s_ParseValDataResponse(Response, 4096);
getchar();
/**/
...
/*
    ...
    sprintf(XmlBuf,
	  "<AtXmlSignalDescription xmlns:atxml=\"ATXML_TSF\">\n"
	  "	<SignalAction>Reset</SignalAction>\n"
	  "	<SignalTimeOut>5.1 s</SignalTimeOut>\n"
      " <SignalResourceName>DevX_1</SignalResourceName>\n"
	  " <SignalSnippet>\n"
        "<Signal name=\"\" Out=\"Meas\" In=\"Cha Ext Chb\">\n"
...
/*
    ...
    Status = atxml_IssueIst(XmlBuf, Response, 4096);
    printf("atxml_IssueIst = %d\n  Response:\n",Status);
    s_ParseErrDbgResponse(Response, 4096);
    s_ParseValDataResponse(Response, 4096);
/**/
/*
...
static void s_ParseValDataResponse(char *Response, int BufferSize)
{
    ATXML_AVAILABILITY AvailArray[20];
    int i;
    char *cptr;
    char  Buf[128];

    memset(AvailArray,0,sizeof(AvailArray));

    // Check Availability
    atxml_ParseAvailability(Response,AvailArray,20);
    for(i=0;i<20;i++)
    {
        if(AvailArray[i].ResourceName[0])
        {
            printf("  <Availability - %s = %s\n",
                   AvailArray[i].ResourceName,
                   AvailArray[i].Availability);
        }
        else 
            break;
    }
    // Check for TestStationStatus
    cptr = Response;
    while(cptr && *cptr)
    {
        cptr = strstr(cptr,"<TestStationStatus");
        while(cptr && (cptr=strstr(cptr,"<ResourceStatus")))
        {
            Buf[0] = '\0';
            sscanf(cptr++,"%120[^>]",Buf);
            printf("  %s...\n",Buf);
    ...
    while(cptr && *cptr)
    {
        cptr = strstr(cptr,"<ReturnData");
        if(cptr && (cptr=strstr(cptr,"<Attribute>")))
        {
            Buf[0] = '\0';
            sscanf(++cptr,"%120[^<]",Buf);
            printf("  <ReturnData> <%s\n",Buf);
