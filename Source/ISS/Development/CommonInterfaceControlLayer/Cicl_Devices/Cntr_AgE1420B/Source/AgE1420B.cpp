...
ATXMLW_WRAP_FNC int Reset(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    for(i = 0; i < s_NumDev; i++)
    {
        ...
        if ((Instno == 0) || (s_DevInfo[i].InstNo == Instno))
        {
			...
			{
                ((CAgE1420B_T *)(s_DevInfo[i].DriverClass))->ResetAgE1420B(Response, BufferSize);
			}
        }
    }
...
ATXMLW_WRAP_FNC int Initialize(int Instno, int ResourceType, char* ResourceName, int Sim, int Dbglvl, ATXMLW_INSTRUMENT_ADDRESS *AddressInfoPtr, ATXMLW_INTF_RESPONSE* Response, int Buffersize)
{
    ...
    if (DE_BUG == 0) {
		sprintf(TmpBuf, "%s", DEBUG_1420Arb);
		DE_BUG = _stat(TmpBuf, &StatBuf) == -1 ? 0 : 1;
	}

    ATXMLW_INTF_RESPONSE tmpResponse[250]; // throw away response string used to call Reset
    ...
    {    // Check for MAX Driver Overflow
        ...
        s_DevInfo[s_NumDev].DriverClass = new CAgE1420B_T(s_DevInfo[s_NumDev].InstNo, s_DevInfo[s_NumDev].ResourceType, s_DevInfo[s_NumDev].ResourceName, s_DevInfo[s_NumDev].Sim,
                                s_DevInfo[s_NumDev].Dbg,  &s_DevInfo[s_NumDev].AddressInfo, Response, Buffersize);
        s_NumDev++;
        Reset(Instno, ResourceType, ResourceName, tmpResponse, 250);
    }
...
ATXMLW_WRAP_FNC int Close(void)
{
    ...
    for(i = 0; i < s_NumDev; i++)
    {
		...
		{
            delete((CAgE1420B_T *)(s_DevInfo[i].DriverClass));
		}
...
ATXMLW_WRAP_FNC int IssueSignal(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_SIGDESC* SignalDescription, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    // 
    ((CAgE1420B_T *)(DriverClass))->IssueSignalAgE1420B(SignalDescription, Response, BufferSize);
...
ATXMLW_WRAP_FNC int Status(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    // 
    ((CAgE1420B_T *)(DriverClass))->StatusAgE1420B(Response, BufferSize);
...
ATXMLW_WRAP_FNC int RegCal(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_CALDATA* CalData)
{
    ...
    // 
    ((CAgE1420B_T *)(DriverClass))->RegCalAgE1420B(CalData);
...
ATXMLW_WRAP_FNC int Ist(int Instno, int ResourceType, char* ResourceName, int Level, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    // 
    ((CAgE1420B_T *)(DriverClass))->IstAgE1420B(Level, Response, BufferSize);
...
ATXMLW_WRAP_FNC int IssueNativeCmds(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_INSTCMD* InstrumentCmds, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    // 
    Status = ((CAgE1420B_T *)(DriverClass))->IssueNativeCmdsAgE1420B(InstrumentCmds,
                            Response, BufferSize);
    if((strstr(InstrumentCmds,"*RST")!=NULL) || (strstr(InstrumentCmds,"RST ")!=NULL) || (strstr(InstrumentCmds,"RST;")!=NULL))    
    {
        Reset(Instno, ResourceType, ResourceName, tmpResponse, 250);
    }
...
ATXMLW_WRAP_FNC int IssueDriverFunctionCall(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_DRVRFNC* DriverFunction, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    // 
    ((CAgE1420B_T *)(DriverClass))->IssueDriverFunctionCallAgE1420B(DriverFunction,
                            Response, BufferSize);
