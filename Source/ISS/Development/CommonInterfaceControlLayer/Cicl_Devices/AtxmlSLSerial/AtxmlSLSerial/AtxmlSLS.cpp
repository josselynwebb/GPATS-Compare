...
//
// Instrument:	SLS232A  <device description> (DMM)
//
...
ATXMLW_WRAP_FNC int Initialize(int Instno, int ResourceType, char* ResourceName, int Sim, int Dbglvl,  ATXMLW_INSTRUMENT_ADDRESS *AddressInfoPtr, ATXMLW_INTF_RESPONSE* Response, int Buffersize)
{
    ...
    if(ResourceType == ATXMLW_RESTYPE_BASE)
    {
        ...
        s_DevInfo[s_NumDev].DriverClass = new CSLS232A_T(s_DevInfo[s_NumDev].InstNo, s_DevInfo[s_NumDev].ResourceType, s_DevInfo[s_NumDev].ResourceName, s_DevInfo[s_NumDev].Sim, 
...
ATXMLW_WRAP_FNC int Close(void)
{
    ...
    for(i = 0; i < s_NumDev; i++)
    {
		...
		{
            delete((CSLS232A_T *)(s_DevInfo[i].DriverClass));
		}
...
ATXMLW_WRAP_FNC int IssueSignal(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_SIGDESC* SignalDescription, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    // 
    ((CSLS232A_T *)(DriverClass))->IssueSignalSLS232A(SignalDescription, Response, BufferSize);
...
ATXMLW_WRAP_FNC int Status(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    ((CSLS232A_T *)(DriverClass))->StatusSLS232A(Response, BufferSize);
...
ATXMLW_WRAP_FNC int RegCal(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_CALDATA* CalData)
{
    ...
    ((CSLS232A_T *)(DriverClass))->RegCalSLS232A(CalData);
    ...
    for(i = 0; i < s_NumDev; i++)
    {
        ...
        if((Instno == 0) || (s_DevInfo[i].InstNo == Instno))
        {
			if(s_DevInfo[i].DriverClass)
                ((CSLS232A_T *)(s_DevInfo[i].DriverClass))->ResetSLS232A(localResponse, localBufferSize);
        }
    }

	if( Response != 0 && BufferSize > 0 )
	{
		strncpy( Response, localResponse, BufferSize );
	}
...
ATXMLW_WRAP_FNC int Ist(int Instno, int ResourceType, char* ResourceName, int Level, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    ((CSLS232A_T *)(DriverClass))->IstSLS232A(Level, Response, BufferSize);
...
ATXMLW_WRAP_FNC int IssueNativeCmds(int Instno, int ResourceType, char* ResourceName, ATXMLW_INTF_INSTCMD* InstrumentCmds, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    Status = ((CSLS232A_T *)(DriverClass))->IssueNativeCmdsSLS232A(InstrumentCmds,
                            Response, BufferSize);
...
ATXMLW_WRAP_FNC int IssueDriverFunctionCall(int Instno, int ResourceType, char* ResourceName,  ATXMLW_INTF_DRVRFNC* DriverFunction, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    ((CSLS232A_T *)(DriverClass))->IssueDriverFunctionCallSLS232A(DriverFunction, Response, BufferSize);
