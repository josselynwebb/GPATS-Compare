...
//
///////////////////////////////////////////////////////////////////////////////
CRi3152_T::CRi3152_T(int Instno, int ResourceType, char* ResourceName, int Sim, int Dbglvl, ATXMLW_INSTRUMENT_ADDRESS *AddressInfoPtr, ATXMLW_INTF_RESPONSE* Response, int Buffersize)
{
    char LclMsg[1024];
    int Status = 0;
    //int  WriteCount;
	ISDODEBUG(dodebug(0, "CRi3152_T(AB)", "Entering function", (char*)NULL));
    // Save Device Info
    m_InstNo = Instno;
    ...
    sprintf(LclMsg,"Wrap-CRi3152 Class called with Instno %d, Sim %d, Dbg %d", 
                                      m_InstNo, m_Sim, m_Dbg);
    ATXMLW_DEBUG(5,LclMsg,Response,Buffersize);

    // Initialize the Ri3152
	IFNSIM(Status = ri3152a_init(m_InitString, false, true, &m_Handle));
	...
	ISDODEBUG(dodebug(0, "CRi3152_T(AB)", "Leaving function", (char*)NULL));
    return;
}
...
CRi3152_T::~CRi3152_T()
{
	...
	//turn output off
	IFNSIM(ri3152a_output(m_Handle, RI3152A_OUTPUT_OFF));

    // Reset the Ri3152
	IFNSIM(ri3152a_reset(m_Handle));
...
int CRi3152_T::StatusRi3152(ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
	...
	ISDODEBUG(dodebug(0, "StatusRi3152(AB)", "Entering function", (char*)NULL));
	...
	ISDODEBUG(dodebug(0, "StatusRi3152(AB)", "Leaving function", (char*)NULL));
    return(Status);
}
...
int CRi3152_T::IssueSignalRi3152(ATXMLW_INTF_SIGDESC* SignalDescription, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
	...
	ISDODEBUG(dodebug(0, "IssueSignalRi3152(AB)", "Entering function", (char*)NULL));
	...
	ISDODEBUG(dodebug(0, "IssueSignalRi3152(AB)", "Leaving function", (char*)NULL));
    return(0);
}
...
int CRi3152_T::RegCalRi3152(ATXMLW_INTF_CALDATA* CalData)
{
    int       Status = 0;
    char      Dummy[1024];
	ISDODEBUG(dodebug(0, "RegCalRi3152(AB)", "Entering function", (char*)NULL));

    // Setup action for the Ri3152
    ATXMLW_DEBUG(5,atxmlw_FmtMsg("Wrap-RegCalRi3152 CalData: %s", CalData),Dummy,1024);

	ISDODEBUG(dodebug(0, "RegCalRi3152(AB)", "Leaving function", (char*)NULL));
    return(0);
}
...
int CRi3152_T::ResetRi3152(ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
	...
	ISDODEBUG(dodebug(0, "ResetRi3152(AB)", "Entering function", (char*)NULL));
    // Reset action for the Ri3152
    ATXMLW_DEBUG(5,atxmlw_FmtMsg("Wrap-ResetRi3152 called "), Response, BufferSize);

	//disable output
	IFNSIM((Status = ri3152a_output(m_Handle, RI3152A_OUTPUT_OFF)));
	if(Status)
	{
	...
	// Reset the Ri3152
	IFNSIM((Status = ri3152a_reset(m_Handle)));
    if(Status)
	{
        ErrorRi3152(Status, Response, BufferSize);
	}
	...
	ISDODEBUG(dodebug(0, "ResetRi3152(AB)", "Leaving function", (char*)NULL));
	return(Status);
}
...
int CRi3152_T::IstRi3152(int Level, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
	...
	ISDODEBUG(dodebug(0, "IstRi3152(AB)", "Entering function", (char*)NULL));
    // Reset action for the Ri3152
	...
	ISDODEBUG(dodebug(0, "IstRi3152(AB)", "Leaving function", (char*)NULL));
    return(Status);
}
...
//
///////////////////////////////////////////////////////////////////////////////
int CRi3152_T::IssueNativeCmdsRi3152(ATXMLW_INTF_INSTCMD* InstrumentCmds, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    int       Status = 0;

	ISDODEBUG(dodebug(0, "IssueNativeCmdsRi3152(AB)", "Entering function", (char*)NULL));
    // Setup action for the Ri3152
	...
	ISDODEBUG(dodebug(0, "IssueNativeCmdsRi3152(AB)", "Leaving function", (char*)NULL));
    return(0);
}
...
int CRi3152_T::IssueDriverFunctionCallRi3152(ATXMLW_INTF_DRVRFNC* DriverFunction, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
	...
	ISDODEBUG(dodebug(0, "IssueDriverFunctionCallRi3152(AB)", "Entering function", (char*)NULL));
    // Setup action for the Ri3152
	...
	ISDODEBUG(dodebug(0, "IssueDriverFunctionCallRi3152(AB)", "Leaving function", (char*)NULL));
    return(0);
}
...
int CRi3152_T::SetupRi3152(ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
	...
	ISDODEBUG(dodebug(0, "SetupRi3152(AB)", "Entering function", (char*)NULL));
    // Setup action for the Ri3152
	...
	if(m_Burst.Exists)
	{
		IFNSIM(Status = ri3152a_operating_mode(m_Handle, RI3152A_MODE_BURST));
		if(Status)
		{
			ErrorRi3152(Status, Response, BufferSize);
		}

		IFNSIM(Status = ri3152a_burst_mode(m_Handle, m_Burst.Int));
		if(Status)
		{
			ErrorRi3152(Status, Response, BufferSize);
		}
		...
		{
			IFNSIM(Status = ri3152a_trigger_source(m_Handle, RI3152A_TRIGGER_INTERNAL));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}

			IFNSIM(Status = ri3152a_trigger_rate(m_Handle, 1/m_BurstRepRate.Real));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
	...
	if(m_Event.Exists)
	{
		...
		else
		{
			...
			if(m_Event.Dim==TRIGGER)
			{
				...
				{
					IFNSIM(Status = ri3152a_operating_mode(m_Handle, RI3152A_MODE_TRIG));
					if(Status)
					{
						ErrorRi3152(Status, Response, BufferSize);
					}
				...
				IFNSIM(Status = ri3152a_trigger_source(m_Handle, RI3152A_TRIGGER_EXTERNAL));
				if(Status)
				{
					ErrorRi3152(Status, Response, BufferSize);
				}
				...
				IFNSIM(Status = ri3152a_set_trigger_level (m_Handle, m_TriggerVolt.Real));
				if(Status)
				{
					ErrorRi3152(Status, Response, BufferSize);
				}
				
				IFNSIM(Status = ri3152a_trigger_slope (m_Handle, (ViBoolean) m_TriggerSlope.Dim));
				if(Status)
				{
					ErrorRi3152(Status, Response, BufferSize);
				}
				...
				{
					IFNSIM(Status = ri3152a_trigger_delay (m_Handle, trigDelayClockCycles));
					if(Status)
					{
						ErrorRi3152(Status, Response, BufferSize);
					}
			...
			else if(m_Event.Dim==GATE)
			{
				...
				IFNSIM(Status =ri3152a_operating_mode (m_Handle, RI3152A_MODE_GATED));
				if(Status)
				{
					ErrorRi3152(Status, Response, BufferSize);
				}
	...
	if(m_Sync.Exists)
	{
		if((strstr(rxBuff,"3152B")) != NULL)
		{
		  IFNSIM(Status = ri3152a_output_sync (m_Handle, RI3152A_SYNC_BIT, 2, 2, 1));
		  if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
		...
		{
			IFNSIM(Status = ri3152a_output_sync (m_Handle, RI3152A_SYNC_BIT, 2, 1, 1));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
	...
	if(m_SignalType.Dim==WAVEFORM)
	{
		IFNSIM(Status=ri3152a_select_waveform_mode (m_Handle, RI3152A_MODE_ARB));
	}
	else
	{
		IFNSIM(Status=ri3152a_select_waveform_mode (m_Handle, RI3152A_MODE_STD));
	}
	if(Status)
	{
		ErrorRi3152(Status, Response, BufferSize);
	...
	switch(m_SignalType.Dim)
	{
		case DC_SIGNAL:
			if((strstr(rxBuff,"3152B")) != NULL)
			{
				...
				IFNSIM(Status = ri3152a_set_amplitude(m_Handle, 16.0));
				if(Status)
				{
					ErrorRi3152(Status, Response, BufferSize);
				}
			...
			else
			{
				...
				IFNSIM(Status = ri3152a_dc_signal(m_Handle, (short) (m_Volt.Real*2/16*100)));
				if(Status)
				{
					ErrorRi3152(Status, Response, BufferSize);
				}
		...
		case AC_SIGNAL:
			...
			IFNSIM(Status = ri3152a_sine_wave(m_Handle, m_Freq.Real, m_Volt.Real, m_DCOffset.Real, 0, 1));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
			...
			//IFNSIM(Status= ri3152a_filter(m_Handle, filter)); cannot be programmed on sine wave
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
		...
		case SQUARE_WAVE:
			...
			IFNSIM(Status = ri3152a_square_wave(m_Handle, m_Freq.Real, m_Volt.Real, m_DCOffset.Real, (ViInt16)m_DutyCycle.Real));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
			...
			IFNSIM(Status= ri3152a_filter(m_Handle, filter));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
		...
		case TRIANGLE_WAVE:
			...
			IFNSIM(Status= ri3152a_triangular_wave(m_Handle, m_Freq.Real, m_Volt.Real, m_DCOffset.Real, 0, 1));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
		...
		case RAMP_SIGNAL:
			...
			IFNSIM(Status= ri3152a_ramp_wave(m_Handle, m_Freq.Real, m_Volt.Real, m_DCOffset.Real, 0.0, percentTime, 100-percentTime));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
		...
		case PULSED_DC:
			...
			IFNSIM(Status= ri3152a_pulse_wave (m_Handle, m_Freq.Real, m_Volt.Real, m_DCOffset.Real, 0.0, m_RiseTime.Real*m_Freq.Real*100, 
		...
		case SINC_WAVE:
			...
			IFNSIM(Status= ri3152a_sinc_wave (m_Handle, m_Freq.Real, m_Volt.Real, m_DCOffset.Real, 10));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
		...
		case EXP_PULSE:
			...
			IFNSIM(Status= ri3152a_exponential_wave (m_Handle, m_Freq.Real, m_Volt.Real, m_DCOffset.Real, (ViInt16)m_Exponent.Real));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
	...
	case WAVEFORM:
			...
			IFNSIM(Status= ri3152a_define_arb_segment(m_Handle, 1, m_StimSize.Int));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
			...
			IFNSIM(Status= ri3152a_load_arb_data(m_Handle, 1, DACarray, (ViInt32) m_StimSize.Int));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
			...
			IFNSIM(Status= ri3152a_output_arb_waveform(m_Handle, 1, 1/m_SampleSpacing.Real, m_Volt.Real, m_DCOffset.Real, RI3152A_CLK_SOURCE_INT));
			if(Status)
			{
				ErrorRi3152(Status, Response, BufferSize);
			}
	...
	ISDODEBUG(dodebug(0, "SetupRi3152(AB)", "Leaving function", (char*)NULL));
    return(0);
}
...
int CRi3152_T::FetchRi3152(ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
    ...
    double  MaxTime = 5000;
    char   *ErrMsg = "";
	ISDODEBUG(dodebug(0, "FetchRi3152(AB)", "Entering function", (char*)NULL));
	...
	ISDODEBUG(dodebug(0, "FetchRi3152(AB)", "Leaving function", (char*)NULL));
    return(0);
}
...
{
	int Status=0;
	ISDODEBUG(dodebug(0, "DisableRi3152(AB)", "Entering function", (char*)NULL));
...
{
	int Status=0;
	ISDODEBUG(dodebug(0, "EnableRi3152(AB)", "Entering function", (char*)NULL));
    // Close action for the Ri3152
    ATXMLW_DEBUG(5,atxmlw_FmtMsg("Wrap-EnableRi3152 called "), Response, BufferSize);
	
	IFNSIM(Status=ri3152a_output(m_Handle, RI3152A_OUTPUT_ON));
	if(Status)
	{
		ErrorRi3152(Status, Response, BufferSize);
	}
...
int  CRi3152_T::ErrorRi3152(int Status, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
	...
	ISDODEBUG(dodebug(0, "ErrorRi3152(AB)", "Entering function", (char*)NULL));
    retval = Status;
    Msg[0] = '\0';
    ...
    if(Status)
    {
        ...
        IFNSIM((Err = ri3152a_error_message(m_Handle, Status, Msg)));
        if(Err)
		{
    ...
    {
        QError = 0;
       	IFNSIM((Err = ri3152a_error_query(m_Handle, &QError, Msg)));
        if(Err != 0)
		{
            break;
		}
	...
	ISDODEBUG(dodebug(0, "ErrorRi3152(AB)", "Leaving function", (char*)NULL));
    return(Status);
}
...
//
///////////////////////////////////////////////////////////////////////////////
int CRi3152_T::GetStmtInfoRi3152(ATXMLW_INTF_SIGDESC* SignalDescription,
                ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
	...
	ISDODEBUG(dodebug(0, "GetStmtInfoRi3152(AB)", "Entering function", (char*)NULL));
	...
	ISDODEBUG(dodebug(0, "GetStmtInfoRi3152(AB)", "Found Action %d", m_Action, (char*)NULL));
	...
	ISDODEBUG(dodebug(0, "GetStmtInfoRi3152(AB)", "Found Resource [%s]", LclResource, (char*)NULL));
    ...
    if((atxmlw_Get1641SignalOut(m_SignalDescription, m_SignalName, m_SignalElement)))
	{
		...
		ISDODEBUG(dodebug(0, "GetStmtInfoRi3152(AB)", "Found SignalOut [%s] [%s]", m_SignalName, m_SignalElement, (char*)NULL));
	}
	...
	ISDODEBUG(dodebug(0, "GetStmtInfoRi3152(AB)", "Leaving function", (char*)NULL));
    return(0);
}
...
{
	ISDODEBUG(dodebug(0, "InitPrivateRi3152(AB)", "Entering function", (char*)NULL));
	m_Volt.Exists=false;
	...
	m_Sync.Exists=false;
	ISDODEBUG(dodebug(0, "InitPrivateRi3152(AB)", "Leaving function", (char*)NULL));
	return;
}
...
{
	ISDODEBUG(dodebug(0, "NullCalDataRi3152(AB)", "Entering function", (char*)NULL));
    m_CalData[0] = 1.0;
	m_CalData[1] = 0.0;
	ISDODEBUG(dodebug(0, "NullCalDataRi3152(AB)", "Leaving function", (char*)NULL));
    return;
}

///////////////////////////////////////////////////////////////////////////////
// Function: SanitizeRi3152
//
...
{
	ISDODEBUG(dodebug(0, "SanitizeRi3152(AB)", "Entering function", (char*)NULL));
	IFNSIM(ri3152a_set_frequency(m_Handle, 100e-6));
	IFNSIM(ri3152a_set_amplitude(m_Handle, 10e-3));
	IFNSIM(ri3152a_output(m_Handle, false));
	ISDODEBUG(dodebug(0, "SanitizeRi3152(AB)", "Leaving function", (char*)NULL));
}
...
void CRi3152_T::Get_Variables(char *Name, ATXMLW_INTF_RESPONSE* Response, int BufferSize)
{
	...
	ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Entering function", (char*)NULL));
	//get element type
	if((atxmlw_Get1641ElementByName(m_SignalDescription, Name, LclElement)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s Element [%s]", Name, LclElement, (char*)NULL));
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "amplitude", &LclDblValue, LclUnit)))
	{
		if(strcmp(LclElement, "Constant")==0) //may be DC-Offset
		{
			if(m_DCOffset.Exists)
			{
				...
				ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s DC-Offset %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
			...
			else //amplitude for DC source
			{
				...
				ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s amplitude %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
				m_Volt.Exists=true;
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "frequency", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s frequency %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
		m_Freq.Exists=true;
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "period", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s period %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
		m_Freq.Exists=true;
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "pulses", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s burst %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
		m_Burst.Exists=true;
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "repetition", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s burst rate %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "nominal", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s trigger leve %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
		...
		if((atxmlw_Get1641StringAttribute(m_SignalDescription, Name, "In", &LclCharValuePtr)))
		{
			ATXMLW_DEBUG(5,atxmlw_FmtMsg("WrapGSI-Found %s In [%s]", Name, LclCharValuePtr), Response, BufferSize);
				ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s In [%s]", Name, LclCharValuePtr, (char*)NULL));
			...
			if((atxmlw_Get1641ElementByName(m_SignalDescription, LclCharValuePtr, temp)))
			{
				...
				if(strcmp(temp, "SignalDelay")==0)
				{
					if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, LclCharValuePtr, "delay", &LclDblValue, LclUnit)))
					{
						...
						ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s trigger delay %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641StringAttribute(m_SignalDescription, Name, "condition", &LclCharValuePtr)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s trigger slope [%s]", LclElement, LclCharValuePtr, (char*)NULL));
	...
	{
		ATXMLW_DEBUG(5,atxmlw_FmtMsg("WrapGSI-Found %s trigger level %E [%s]", LclElement, LclDblValue,LclUnit), Response, BufferSize);
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s trigger level %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
		m_TriggerVolt.Exists=true;
		m_TriggerVolt.Real=LclDblValue;
		m_Event.Exists=true;
		m_Event.Dim=GATE;
	...
	{
		ATXMLW_DEBUG(5,atxmlw_FmtMsg("WrapGSI-Found %s trigger slope [%s]", LclElement, LclCharValuePtr), Response, BufferSize);
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s tslope [%s]", LclElement, LclCharValuePtr, (char*)NULL));
		m_TriggerSlope.Exists=true;
		if(strcmp(LclCharValuePtr, "GT")==0)
		{
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "cutoff", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s bandwidth %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "maxTime", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s Max-Time %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "dutyCycle", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s Duty-Cycle %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "riseTime", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s RiseTime %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "fallTime", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s FallTime %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "exponent", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s exponent %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "samplingInterval", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s Sample-Spacing %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641StringAttribute(m_SignalDescription, Name, "points", &LclCharValuePtr)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s points [%s]", LclElement,  &LclCharValuePtr, (char*)NULL));
	...
	if((atxmlw_Get1641DoubleAttribute(m_SignalDescription, Name, "resistance", &LclDblValue, LclUnit)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s Test-Equip-Imp %E [%s]", LclElement, LclDblValue,LclUnit, (char*)NULL));
	...
	if((atxmlw_Get1641StringAttribute(m_SignalDescription, Name, "syncOut", &LclCharValuePtr)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s syncOut [%s]", LclElement, LclCharValuePtr, (char*)NULL));
	...
	if((atxmlw_Get1641StringAttribute(m_SignalDescription, Name, "In", &LclCharValuePtr)))
	{
		...
		ISDODEBUG(dodebug(0, "Get_VariablesRi3152(AB)", "Found %s Element [%s]",LclCharValuePtr, LclElement, (char*)NULL));
	...
	ISDODEBUG(dodebug(0, "Get_VariableRi3152(AB)", "Leaving function", (char*)NULL));
	return;
}
//++++/////////////////////////////////////////////////////////////////////////
