...
//
// DoTargetParse:	This program will parse through the argv list, checking
...
int DoTargetParse(int argc, char *argv[])
{
	/* Check if all the arguments have already been parsed.
	Should expect that the file name and the frame reference have been
	parsed.*/
	dodebug(0,"DoTargetParse()","Entering function in targeter class", (char*)NULL);
	if (EtmInfo.NotAtlas == FALSE) {

		if (ParseArgumentsAtlas(argc, argv, READ_TARGET)) {
			return(NAM_ERROR);
		}
	}
	else {

		if (ParseArguments(argc, argv, READ_TARGET)) {
			return(NAM_ERROR);
		}
	}
	...
	dodebug(0,"DoTargetParse()","Leaving function in targeter class", (char*)NULL);
	return(SUCCESS);
}

//
// PerformTarget:	This program will first check to see if the monitoring
...
//						by sending the TARGET_MSG, then finally send the reader
...
void PerformTarget(void)
{
	
	dodebug(0,"PerformTarget()","Entering function in targeter class", (char*)NULL);
	...
	dodebug(0,"PerformTarget()","Leaving function in targeter class", (char*)NULL);
	return;
}

//
// SendTargetInfo:	This program will keep looping through getting request
...
void SendTargetInfo(void)
{
	...
	dodebug(0,"SendTargetInfo()","Entering function in targeter class", (char*)NULL);
	while (LoopForAnotherReading) {
		...
		switch(PipeInfo.MessageValue) {
			...
			case FUNCTIONAL_CMD:
				...
				if (FormatSendMessage(TARGET_CMD)) {
					return;
				}
			...
			default:
				...
				EtmInfo.ReturnValue = NAM_ERROR;
				dodebug(0, "SendTargetInfo()", "PipeInfo.MessageValue = %d",
	...
	dodebug(0,"SendTargetInfo()","Leaving function in targeter class", (char*)NULL);
	return;
}
