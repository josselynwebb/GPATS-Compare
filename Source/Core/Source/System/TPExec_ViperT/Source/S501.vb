...
<System.Runtime.InteropServices.ProgId("S501_NET.S501")> Public Class S501
	'**************************************************************
	'* Third Echelon Test Set (TETS) Software Module              *
	'*                                                            *
	'* Nomenclature   : S501 Switch Utility                       *
	...
	Public Sub swClose(ByRef nPinA As Short, ByRef nPinB As Short)
		...
		'Set Display
		gFrmMain.txtInstrument.Text = "Switch S501"
		gFrmMain.txtCommand.Text = "swClose"
		
		'Error Check Pin Numbers
		If nPinA < 1 Or nPinA > 2 Then
			Echo("SWITCH S501 PROGRAMMING ERROR:  Command S501.swClose nPinA argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
		If nPinB < 3 Or nPinB > 10 Then
			Echo("SWITCH S501 PROGRAMMING ERROR:  Command S501.swClose nPinB argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
	...
	Public Sub swOpen(Optional ByRef nPinA As Short = 0, Optional ByRef nPinB As Short = 0)
		...
		'Error Check Pin Numbers
		If nPinA < 1 Or nPinA > 2 Then
			Echo("SWITCH S501 PROGRAMMING ERROR:  Command S501.swOpen nPinA argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
		If nPinB < 3 Or nPinB > 10 Then
			Echo("SWITCH S501 PROGRAMMING ERROR:  Command S501.swOpen nPinB argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
	...
	Public Sub swOpenAll()
		...
		If Not bSimulation Then WriteSW("OPEN 1.4000-4017")
    End Sub
End Class