...
<System.Runtime.InteropServices.ProgId("cOSCOPE_NET.cOSCOPE")> Public Class cOSCOPE
    'bbbb  Made the following changes to the DSO class
    'All R/W commands go through the cicl
    'Changed volt range max from 40V to 50V
    'Auto Mode is not setting voltage range, so added the following command
    '     WriteMsg OSCOPE, "VOLT1:RANG " & sVoltRange
    'Also changed default dVoltRange from 4V to 20V
	
	'bbbb 5-23-2011
	'Made the following changes in order to maintain compatability with tets
	'created a new ArmVPP2 with added parameter dTriggerDelay
	'changed all dVoltRange back to 4V default
    ...
    Public Sub ArmVPP(Optional ByRef nChannel As Integer = 1, Optional ByRef dVoltRange As Double = 4,
                      ...
                      Optional ByRef sTriggerSource As String = "INP1", Optional ByRef nInputImp As Integer = 1000000.0,
                      Optional ByRef sInputCoupling As String = "DC", Optional ByRef sLPAS As String = "OFF",
                      Optional ByRef sHPAS As String = "OFF")
        ArmVPP2(nChannel, dVoltRange, dVoltOffSet, dTimeRange, dTriggerLevel, sTriggerSlope, sTriggerSource, nInputImp, sInputCoupling, sLPAS, sHPAS)
    End Sub
	
    Public Sub ArmVPP2(Optional ByRef nChannel As Integer = 1, Optional ByRef dVoltRange As Double = 4,
                       Optional ByRef dVoltOffSet As Double = 0, Optional ByRef dTimeRange As Double = 0.001,
                       Optional ByRef dTriggerLevel As Double = 0, Optional ByRef sTriggerSlope As String = "POS",
                       Optional ByRef sTriggerSource As String = "INP1", Optional ByRef nInputImp As Integer = 1000000,
                       Optional ByRef sInputCoupling As String = "DC", Optional ByRef sLPAS As String = "OFF",
                       Optional ByRef sHPAS As String = "OFF", Optional ByRef dTriggerDelay As Double = 0.00000003)
        ...
        'bbbb added trigger delay
        If dTriggerDelay < 0.00000003 Or dTriggerDelay > 0.16 Then
            Echo("OSCOPE PROGRAMMING ERROR:  Command cmdOSCOPE.ArmVPP sTriggerDelay argument out of range.")
            Echo("RANGE: 0.000000030 to 0.160")
            Err.Raise(-1001)
            Exit Sub
        End If
        ...
        If Not bSimulation Then
            ...
            ' viClear (nInstrumentHandle(OSCOPE))
            WriteMsg(OSCOPE, "SYST:LANG COMP")
            Delay(1)
            WriteMsg(OSCOPE, "*CLS")
            WriteMsg(OSCOPE, "*RST")

            'bbbb added to change the output buffer from the last measured value
            ...
            WriteMsg(OSCOPE, "MEAS:VOLT:AC? (@INP1)") ' should measure 0, without a trigger
            Delay(0.2)
            WriteMsg(OSCOPE, "*CLS")
            WriteMsg(OSCOPE, "*RST")
            'bbbb
	...
	Public Function dMeasFetch(Optional ByRef nChannel As Short = 1) As Double
		...
		If nChannel < 1 Or nChannel > 2 Then
			...
			Echo("RANGE: 1 TO 2")
			dMeasFetch = 9.9E+37 ' was dMeasFretch bbbb
			Err.Raise(-1001)
			Exit Function
		End If
		...
		If Not bSimulation Then
            'bbbb   nErr = hpe1428a_readInstrData(nInstrumentHandle(OSCOPE), numberBytesToRead:=8100, ReadBuffer:=ReadBuffer, numBytesRead:=Nbr)
			...
			If nErr <> 0 Then
				...
				Exit Function
			End If
			'bbbb   sData = Left(ReadBuffer, Nbr)
			If sData = "" Then sData = "0" 'bbbb
    ...
    Public Function dMeasFreq(ByRef sAutoScale As String, ByRef sINP As String, Optional ByRef dVoltRange As Double = 4,
        ...
        If Not bSimulation Then
            ...
            Select Case sAutoScale
                Case "ON", "AUTO"
                    'bbbb auto is not setting voltage range
                    WriteMsg(OSCOPE, "VOLT1:RANG " & sVoltRange)
        ...
        End If
        'Make Mesurement
        If UserEvent = ABORT_BUTTON Then Err.Raise(USER_EVENT + ABORT_BUTTON) : GoTo ExitFunction 'bbbb
        If Not bSimulation Then
