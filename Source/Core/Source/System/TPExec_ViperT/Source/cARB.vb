...
<System.Runtime.InteropServices.ProgId("cARB_NET.cARB")> Public Class cARB
    ...
    Private Function ErrorCheck(ByRef sShape As String, ByRef dAmplitude As Double, ByRef dFrequency As Double, ByRef sLowPassFilter As String, ByRef iStep As Short, ByRef iScale As Short, ByRef dDCOffset As Double, ByRef lRampPoints As Integer) As Boolean
...
'        Dim TmpName As String
    ...
    Public Sub DownloadSequence(ByRef SequenceName As String, ByRef SegmentList() As String, ByRef DwellCounts() As Integer)
        ...
        If ((lMaxIndexDwell - lMinIndexDwell + 1) <> lPointCount) And ((lMaxIndexDwell - lMinIndexDwell + 1) <> 1) Then
            Echo("There are an invalid number of dwell counts specified." & vbCrLf & "There must be the same number of Dwell counts as there are Segments or there must be exactly one dwell count.")
            Err.Raise(1000, , "There are an invalid number of dwell counts specified." & vbCrLf & "There must be the same number of Dwell counts as there are Segments or there must be exactly one dwell count.")
            Exit Sub
        End If
