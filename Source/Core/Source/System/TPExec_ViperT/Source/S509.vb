...
<System.Runtime.InteropServices.ProgId("S509_NET.S509")> Public Class S509
	'**************************************************************
	...
	Public Sub swClose(ByRef nPinA As Short, ByRef nPinB As Short)
		...
		'Set Display
		gFrmMain.txtInstrument.Text = "Switch S509"
		gFrmMain.txtCommand.Text = "swClose"
		
		'Error Check Pin Numbers
		If nPinA < 1 Or nPinA > 2 Then
			Echo("SWITCH S509 PROGRAMMING ERROR:  Command S509.swClose nPinA argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
		If nPinB < 3 Or nPinB > 10 Then
			Echo("SWITCH S509 PROGRAMMING ERROR:  Command S509.swClose nPinB argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
	...
	Public Sub swOpen(Optional ByRef nPinA As Short = 0, Optional ByRef nPinB As Short = 0)
		...
		'Error Check Pin Numbers
		If nPinA < 1 Or nPinA > 2 Then
			Echo("SWITCH S509 PROGRAMMING ERROR:  Command S509.swOpen nPinA argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
		If nPinB < 3 Or nPinB > 10 Then
			Echo("SWITCH S509 PROGRAMMING ERROR:  Command S509.swOpen nPinB argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
	...
	Public Sub swOpenAll()
		...
		If Not bSimulation Then WriteSW("OPEN 2.4300-4317")
    End Sub
End Class