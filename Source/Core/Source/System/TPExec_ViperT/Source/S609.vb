...
<System.Runtime.InteropServices.ProgId("S609_NET.S609")> Public Class S609
	'**************************************************************
	...
	Public Sub swClose(ByRef nPinA As Short, ByRef nPinB As Short)
		...
		'Set Display
		gFrmMain.txtInstrument.Text = "Switch S609"
		gFrmMain.txtCommand.Text = "swClose"
		
		'Error Check Pin Numbers
		If nPinA < 1 Or nPinA > 1 Then
			Echo("SWITCH S609 PROGRAMMING ERROR:  Command S609.swClose nPinA argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
		If nPinB < 2 Or nPinB > 3 Then
			Echo("SWITCH S609 PROGRAMMING ERROR:  Command S609.swClose nPinB argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
	...
	Public Sub swOpen(Optional ByRef nPinA As Short = 0, Optional ByRef nPinB As Short = 0)
		...
		'Error Check Pin Numbers
		If nPinA < 1 Or nPinA > 1 Then
			Echo("SWITCH S609 PROGRAMMING ERROR:  Command S609.swOpen nPinA argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
		If nPinB < 2 Or nPinB > 3 Then
			Echo("SWITCH S609 PROGRAMMING ERROR:  Command S609.swOpen nPinB argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
	...
	Public Sub swOpenAll()
		...
		If Not bSimulation Then WriteSW("OPEN 2.2200-2201")
    End Sub
End Class