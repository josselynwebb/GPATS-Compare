Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("S601_NET.S601")> Public Class S601
	'**************************************************************
	'* Third Echelon Test Set (TETS) Software Module              *
	'*                                                            *
	'* Nomenclature   : S601 Switch Utility                       *
	...
	Public Sub swClose(ByRef nPinA As Short, ByRef nPinB As Short)
		...
		'Set Display
		gFrmMain.txtInstrument.Text = "Switch S601"
		gFrmMain.txtCommand.Text = "swClose"
		
		'Error Check Pin Numbers
		If nPinA < 1 Or nPinA > 1 Then
			Echo("SWITCH S601 PROGRAMMING ERROR:  Command S601.swClose nPinA argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
		If nPinB < 2 Or nPinB > 3 Then
			Echo("SWITCH S601 PROGRAMMING ERROR:  Command S601.swClose nPinB argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
	...
	Public Sub swOpen(Optional ByRef nPinA As Short = 0, Optional ByRef nPinB As Short = 0)
		...
		'Error Check Pin Numbers
		If nPinA < 1 Or nPinA > 1 Then
			Echo("SWITCH S601 PROGRAMMING ERROR:  Command S601.swOpen nPinA argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
		If nPinB < 2 Or nPinB > 3 Then
			Echo("SWITCH S601 PROGRAMMING ERROR:  Command S601.swOpen nPinB argument out of range.")
			Err.Raise(-1000)
			Exit Sub
		End If
	...
	Public Sub swOpenAll()
		...
		If Not bSimulation Then WriteSW("OPEN 1.2000-2001")
    End Sub
End Class