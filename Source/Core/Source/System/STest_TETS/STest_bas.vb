...
Public Module STestMain
    ...
    '  Declare Function gt50000_init Lib "gt50000.dll" (ByVal resourceName As String, ByVal idQuery As Short, ByVal resetDevice As Short, ByRef instrumentHandle As Integer) As Integer
    Declare Function gt50000_init Lib "gt50000.dll" (ByVal resourceName As String, ByVal idQuery As Integer, ByVal resetDevice As Integer, ByRef instrumentHandle As Integer) As Integer
    ...
    Public Const FAILED As Integer = 0 '  0
    Public Const PASSED As Integer = -1 ' -1
    ...
    Public Const RFCOUNTER As Short = 22
    Public Const RFDOWN As Short = 23 ' gt55210a or eip1315a  
    Public Const RFMODANAL As Short = 24 ' modulation analyzer
    Public Const RFSYN As Short = 25
    Public Const LAST_RF_INST = RFSYN

    Public Const LAST_INSTRUMENT = RFSYN

    Public Const CBL_W24 = LAST_INSTRUMENT + 5
    Public Const CBL_W25 = LAST_INSTRUMENT + 6
    Public Const CBL_W21 = LAST_INSTRUMENT + 7
    ...
    Public InstFileName(LAST_INSTRUMENT + 7) As String
    Public instrumentHandle(LAST_INSTRUMENT + 7) As Integer
    Public CardStatus(LAST_INSTRUMENT + 7) As Integer
    Public IDNResponse(LAST_INSTRUMENT + 7) As String
    Public InstrumentSpec(LAST_INSTRUMENT + 7) As String
    Public InstrumentDescription(LAST_INSTRUMENT + 7) As String '(+7) Supports Cables and RF Meas instruments
    Public InstrumentInitialized(LAST_INSTRUMENT + 7) As Integer
    Public InstrumentStatus(LAST_INSTRUMENT + 7) As Integer
    ...
    Public sTestName(LAST_INSTRUMENT + 7) As String
    ...
    Public sPathLossPath As String
    ...
    Public sAts_INI_path As String ' just the path
    ...
    '    SYSTEM_SURVEYT=C:\Program Files (x86)\ATS\System Test\SYSTEM_SURVEY.EXE
...
#End If
    ...
    Sub ResetSystem(ByVal ResetSupplies As Boolean)
        ...
        'If RFOptionInstalled Then
        '    If GIGA_DOWN = True Then
        '        InitStatus = atxmlDF_gt55210a_setLO("RF_CNTR_1", 0, MANUAL, 0, IFOUT)
        '    Else
        '        InitStatus = atxmlDF_eip1315a_setLO("RF_CNTR_1", 0, MANUAL, 70, DETOUT)
        '    End If
    ...
    Sub DisplaySetup(ByVal Caption As String, ByVal Graphic As String, ByVal LineCount As Integer, Optional ByVal BackOK As Integer = False, Optional ByVal BackNo As Integer = 0, Optional ByVal BackCount As Integer = 0)
        ...
        Else
            frmDirections.picGraphic.Image = LoadPicture(ProgramPath & "GRAPHICS\TETS APN_PCU.jpg")
        End If
    ...
    Function FetchMeasurement(ByVal Instrument As Integer, ByRef MeasComment As String, ByRef LowLimit As String, ByRef HighLimit As String, ByRef Units As String, Optional ByVal iFailingInstrumentIndex As Integer = 0, Optional ByVal sTestNum As String = "", Optional ByVal sSwitchCallout As String = "") As Double
        ...
        Else
            kHzFlg = False
        End If
        nSystErr = ReadMsg(Instrument, StrMeasurement) ' gets the measurement from the selected instrument
...
EndOfLoop:
    ...
    Function InitPowerSupplies() As Integer
        ...
        For Supply = 1 To 10
            ...
            Else                'Identification FAILED
                ...
                RegisterFailure(PPU, sPPU_Test_Number, sComment:="    Identification Failure for Supply " & CStr(Supply) & ".")
    ...
    Sub PerformSelfTest(ByVal iButton As Integer)
        ...
        Dim EndToEndTesting As Integer
        ...
        Select Case iButton
            ...
            Case Else
                ...
                If fnSAIFinstalled(True) = False Then ' make sure the saif is installed for any test
                    ...
                    DisplaySetup(sMsg, "TETS SAIF_CON.jpg", 1)
                    If AbortTest = True Then
                        Echo("Test Aborted" & vbCrLf)
                        GoTo TestComplete
                    End If
...
LoopOnEnd:
        ...
        For iTestNumber = StartInst To StopInst '  bb was using "For iSelGUI = StartInst To StopInst"  caused warnings?
            ...
            Select Case iSelGUI
                ...
                Case 21 : iSelInst = frmSTest.sscTest_21.Tag
                Case 22 : iSelInst = frmSTest.sscTest_22.Tag
                Case 23 : iSelInst = frmSTest.sscTest_23.Tag
                Case 24 : iSelInst = frmSTest.sscTest_24.Tag
                Case 25 : iSelInst = frmSTest.sscTest_25.Tag
            End Select
            SwitchTest = False
...
Retest:     frmSTest.timTimer.Enabled = True
            ...
            Select Case iSelGUI
                ...
                Case 24 : frmSTest.InstrumentLabel_24.ForeColor = Color.Black 'rf
                Case 25 : frmSTest.InstrumentLabel_25.ForeColor = Color.Black 'rf
            End Select
            ...
            ElseIf (TestStatus <> iSelInst) Then                    'Test failed due to a failure in a different instrument
                ...
                End If
                InstrumentStatus(iSelInst) = UNTESTED
                '   MsgBox("The " & InstrumentDescription(iSelInst) & " was unable to be tested due to a failure in another module. " & vbCrLf & "(" & InstrumentDescription(TestStatus) & ")" & vbCrLf & "Read 'Details' box or System Log file for more information.")
...
ReportToFHDB:
    ...
    Sub EchoSlotTemperature(TestX As Integer)
        ...
        Select Case TestX
            ...
            Case 19  ' CANBUS
            Case 20  ' APROBE
            Case 21 : ChasX = 2 : SlotX = 11 ' RFPM
            Case 22 : ChasX = 2 : SlotX = 9  ' RFCOUNTER
            Case 23 : ChasX = 2 : SlotX = 7  ' RFDOWN CONVERTER
            Case 24 : ChasX = 2 : SlotX = 10 ' RF MODULATION ANALYZER
            Case 25 : ChasX = 2 : SlotX = 5  ' RF SYNTHESIZER
        End Select
        If (ChasX < 1) Or (ChasX > 2) Then Exit Sub

        GetOneDataDumpFromBothChassis()

        '2nd Get and print the temperature
        Select Case TestX
            ...
            Case 22, 23 ' RF Counter or RFDC slots 7,9
                TempX = GetSlotTemperature(2, 7)
                TempX = GetSlotTemperature(2, 9)

            Case 24 'RF Modulation Analyzer Slots 7,10
                TempX = GetSlotTemperature(2, 7)
                TempX = GetSlotTemperature(2, 10)

            Case 25 'RF SYN Slots 4,5,6
    ...
    Function CheckOptionInstalled(ByVal sOpt As String) As Boolean
        'DESCRIPTION:
        '   This routine determines if the ATS RF option is installed
        ...
        If LogFileStartup = 0 Then
            ...
            Try
                ...
                If Err.Number <> 0 Then
                    ...
                    Err.Clear()
                Else
                    PrintLine(FileHandle, "ATS-TETS Serial Number: " & sUUT_Serial_No)
    ...
    Function InitMessageBasedInstrument(ByVal InstrumentToInit As Integer) As Integer
        ...
        'this should only open the RF instruments.  Everything else goes through the cicl
        Select Case InstrumentToInit
            Case CBL_W21 ' RF COUNTER, RFDOWN 'RFSYN, RFPM, 
                'XXX-00-N01
                InitStatus = viOpen(SessionHandle, DevSpec, VI_NULL, VI_NULL, IHandle)
                If InitStatus <> VI_SUCCESS Then
                    sMsg = sInstrumentIDCode(InstrumentToInit) & "-00-N01"
                    Echo(sMsg & Space(69 - Len(sMsg)) & "FAILED")
                ...
                Else
                    sMsg = sInstrumentIDCode(InstrumentToInit) & "-00-N01"
                    Echo(sMsg & Space(69 - Len(sMsg)) & "PASSED")
                End If
        End Select

        Select Case InstrumentToInit
            Case CBL_W21 ' RFCOUNTER, RFDOWN 'RFSYN,  RFPM, 
                FormatResultLine(InstrumentDescription(InstrumentToInit) & " initialization ", True)
        ...
        Select Case InstrumentToInit
            ...
            Case Is <= LAST_RF_INST
                nSystErr = WriteMsg(InstrumentToInit, "*RST")
                Delay(1)
        ...
        If IDSpec = "E1428A" Then
            If InStr(Trim(IDList(MANUF)) & Trim(IDList(MODEL)), "ZT1428") <> 0 Then
                ...
                InstrumentDescription(OSCOPE) = "Teradyne ZT1428 Digitizing Oscilloscope (SC Slot 1)"
            ElseIf IDSpec = "ZT4628A" Then
                If InStr(Trim(IDList(MANUF)) & Trim(IDList(MODEL)), "ZT1428") <> 0 Then
                    IDSpec = "ZT4628"
                    IDNResponse(OSCOPE) = "ZT4628" ' change instrument to ZTEC 1428
                    InstrumentDescription(OSCOPE) = "Teradyne ZT4628 Digitizing Oscilloscope (SC Slot 1)"
                End If
            End If
        ...
        Else
            Echo("Could not find instrument ID string: """ & Trim(IDList(MANUF)) & IDSpec & """ in """ & Trim(sMsg) & """")
    ...
    Sub InitVar()
        ...
        IDNResponse(DMM) = "E1412A" 'MAY be changed to "4152A" in InitMessageBasedInstrument for TETS
        IDNResponse(COUNTER) = "E1420B"
        IDNResponse(ARB) = "E1445A"
        IDNResponse(RFPM) = "E1416A"
        IDNResponse(RFSYN) = "50000"   'Giga-tronics, 50000
        IDNResponse(OSCOPE) = "E1428A" 'MAY be changed to "ZT1428" or "ZT4628" in InitMessageBasedInstrument for TETS
        IDNResponse(SYN_RES) = "65CS4"
        IDNResponse(FGEN) = "3152"
        IDNResponse(RFCOUNTER) = "E1420B"
        IDNResponse(RFDOWN) = "1315A" ' may get changed to gt55210a in InitMessageBasedInstrument
        IDNResponse(RFMODANAL) = "Boonton Model 8701"  'Booton\sModel\s8701,

        'sec chassis
        InstrumentSpec(OSCOPE) = "VXI::17::INSTR"    'dso
        InstrumentSpec(COUNTER) = "VXI::18::INSTR"   'c/t
        InstrumentSpec(FGEN) = "VXI::19::INSTR"      'fgen
        InstrumentSpec(RFSYN) = "VXI0::20::INSTR"    'gigatronics 50000  RF synthesizer
        InstrumentSpec(RFDOWN) = "VXI::23::INSTR"     'EIP 1315A or gt55210a RF Downconverter 
        InstrumentSpec(RFCOUNTER) = "VXI::25::INSTR" 'rf c/t
        InstrumentSpec(RFMODANAL) = "VXI::27::INSTR"    'boonton 8701 modulation meter
        InstrumentSpec(RFPM) = "VXI::28::INSTR"      'pwr meter
        InstrumentSpec(SYN_RES) = "VXI::30::INSTR"   'S/R

        'pri chassis
        InstrumentSpec(SWITCH1) = "VXI::38::INSTR" 'switch controller
        InstrumentSpec(DMM) = "VXI::44::INSTR"     'dmm
        InstrumentSpec(ARB) = "VXI::48::INSTR"     'arb

        InstrumentDescription(END_TO_END) = "End to End"
        InstrumentDescription(DMM) = "HP E1412A Digital Multimeter (PC Slot 11)"
        ...
        InstrumentDescription(RFPM) = "HP E1416A RF Power Meter (SC Slot 11)"
        InstrumentDescription(RFSYN) = "Giga-tronics 50000A RF Synthesizer (SC Slot 5-6)"
        InstrumentDescription(RFCOUNTER) = "HP E1420B Counter/Timer (SC Slot 9)"
        InstrumentDescription(RFDOWN) = "Giga-tronics GT55210A RF Downconverter (Slot 7)"
        ''InstrumentDescription(RFDOWN) = "EIP 1315A RF Downconverter" ' - No longer supported"
        InstrumentDescription(RFMODANAL) = "Boonton 8701 Modulation Meter (SC Slot 10)"
        InstrumentDescription(OSCOPE) = "HP 1428A Digitizing Oscilloscope (SC Slot 1)"
        ' InstrumentDescription(OSCOPE) = "ZTEC 1428 Digitizing Oscilloscope"
        ' InstrumentDescription(OSCOPE) = "ZTEC 4628 Digitizing Oscilloscope"
        InstrumentDescription(PPU) = "Freedom Power Systems FPU/PPU"
        ...
        InstrumentDescription(MIL_STD_1553) = "Ballard Lx1553-5 (CIC Slot 8)"
        ...
        InstrumentDescription(RS422) = "RS422 9 pin SeaLevel Systems 7404 COM1 (CIC Slot3)"
        InstrumentDescription(RS232) = "RS232 9 pin COM2 (CIC Slot 1 computer)"
        ...
        InstFileName(RFCOUNTER) = "RFC" ' counter
        InstFileName(RFDOWN) = "RFD" ' down converter
        InstFileName(RFMODANAL) = "RFM" ' modulation meter
        InstFileName(RFSYN) = "RFS" 'synthesizer
        InstFileName(RFPM) = "RFP" ' power meter
        ...
        InstFileName(APROBE) = "ANALOG PROBE"


        sGuiLabel(END_TO_END) = "End to End"
        sGuiLabel(MIL_STD_1553) = "MIL-STD-1553 Interface  (CIC Slot 8)"
        sGuiLabel(DIGITAL) = "Digital Test Subsystem (DTS PC Slots 1-4)"
        ...
        sGuiLabel(PPU) = "Programmable Power Unit (PPU Slots C,D)"
        ...
        sGuiLabel(RS422) = "RS422 COM1 (CIC Slot 3)"                    ' (one 9 pin connector, also in the serial card)
        sGuiLabel(RS232) = "RS232 COM2 (PIC CIC Slot 1)"                ' (one 9 pin connector, via CIC)
        ...
        sGuiLabel(RFPM) = "RF Power Meter (SC Slot 11)"
        sGuiLabel(RFSYN) = "RF Stimulus (SC Slots 4-6)"
        sGuiLabel(RFCOUNTER) = "RF Counter (SC Slot 9)"
        sGuiLabel(RFDOWN) = "RF Downconverter (SC Slot 7)"
        sGuiLabel(RFMODANAL) = "RF Modulation Meter (SC Slot 10)"
        ...
        sInstrumentIDCode(RFCOUNTER) = "RCT"
        sInstrumentIDCode(RFDOWN) = "REC"
        sInstrumentIDCode(RFMODANAL) = "RMA"
        ...
        ResourceName(RFCOUNTER) = "RF_CNTR_1"
        ResourceName(RFDOWN) = "RFGEN_1"
        ResourceName(RFMODANAL) = "RF_MEASAN_1"
    ...
    Function WaitForResponse(ByVal instrumentIndex As Short, ByVal ProgressIncr As Single) As Integer
        'DESCRIPTION
        ...
        For count = 1 To 300 'This allows for a wait of about 60 seconds max
            Delay(0.2)
            If frmSTest.proProgress.Value + CInt(ProgressIncr) < 100 Then
                frmSTest.proProgress.Value += CInt(ProgressIncr)
            End If
            Select Case instrumentIndex
                Case CBL_W21 '  RFCOUNTER, RFDOWN, RFMODANAL 'RFSYN, RFPM, 
    ...
    Function ReadMsg(ByVal InstrumentIndex As Integer, ByRef ReturnMessage As String) As Integer
        ...
        'DESCRIPTION:
        'RF and DSO only
        '   This Routine is using VB conventions to
        '   facilitate clean Word Serial read communications to message based instruments.
        'NonRF
        '   This Routine is a CICL pass through to the VISA layer using VB conventions to
        '   facilitate clean Word Serial read communications to message based instruments.
        'PARAMETERS:
        ...
        Select Case InstrumentIndex
            Case CBL_W21 ' RFCOUNTER, RFDOWN, RFMODANAL 'RFSYN, RFPM, 
                ...
                nSystErr = viRead(instrumentHandle(InstrumentIndex), ReturnMessage, 256, retCount)
                If nSystErr <> 0 Then
                    Select Case nSystErr ' vi read error?
                        '   Case VI_SUCCESS : S = "The operation completed successfully and the END indicator was received (for interfaces that have END indicators). This completion code is returned regardless of whether the termination character is received or the number of bytes read is equal to count."
                        Case VI_SUCCESS_TERM_CHAR : S = "The specified termination character was read but no END indicator was received. This completion code is returned regardless of whether the number of bytes read is equal to count."
                        Case VI_SUCCESS_MAX_CNT : S = "The number of bytes read is equal to count. No END indicator was received and no termination character was read."
                        Case VI_ERROR_INV_OBJECT : S = "The given session reference is invalid."
                        Case VI_ERROR_NSUP_OPER : S = "The given vi does not support this operation."
                        Case VI_ERROR_TMO : S = "Timeout expired before operation completed."
                        Case VI_ERROR_RAW_WR_PROT_VIOL : S = "Violation of raw write protocol occurred during transfer."
                        Case VI_ERROR_RAW_RD_PROT_VIOL : S = "Violation of raw read protocol occurred during transfer."
                        Case VI_ERROR_INV_SETUP : S = "Unable to start read operation because setup is invalid (due to attributes being set to an inconsistent state)."
                        Case VI_ERROR_NCIC : S = "The interface associated with the given vi is not currently the controller in charge."
                        Case VI_ERROR_NLISTENERS : S = "No-listeners condition is detected (both NRFD and NDAC are unasserted)."
                        Case VI_ERROR_ASRL_PARITY : S = "A parity error occurred during transfer."
                        Case VI_ERROR_ASRL_FRAMING : S = "A framing error occurred during transfer."
                        Case VI_ERROR_ASRL_OVERRUN : S = "An overrun error occurred during transfer. A character was not read from the hardware before the next character arrived."
                        Case VI_ERROR_IO : S = "An unknown I/O error occurred during transfer."
                        Case VI_ERROR_RSRC_LOCKED : S = "Specified operation could not be performed because the resource identified by vi has been locked for this kind of access."
                        Case VI_ERROR_CONN_LOST : S = "The I/O connection for the given session has been lost."
                        Case VI_ERROR_OUTP_PROT_VIOL : S = "Device reported an output protocol error during transfer."
                        Case VI_ERROR_BERR : S = "Bus error occurred during transfer."
                    End Select

                    If (InstrumentIndex = RFMODANAL) And (nSystErr = VI_ERROR_TMO) And (LastWriteMessage = "*IDN?") Then
                    ElseIf (InstrumentIndex = RFSYN) And (nSystErr = VI_ERROR_TMO) And (LastWriteMessage = "SYST:ERR?") Then
                    Else
                        '   Echo("--" & InstrumentDescription(InstrumentIndex))
                        Echo("-- Visa read message error, " & InstrumentDescription(InstrumentIndex) & vbCrLf & "-- " & S)
                    End If
    ...
    Function WriteMsg(ByVal InstrumentIndex As Integer, ByVal MessageToSend As String) As Integer
        ...
        'EXAMPLE:
        '   nSystErr = WriteMsg(DMMHandle, "*TST?")  '* This will run a self test on the DMM
        ...
        Select Case InstrumentIndex
            Case CBL_W21
                nSystErr = viWrite(instrumentHandle(InstrumentIndex), MessageToSend, CInt((MessageToSend).Length), retCount)
                If nSystErr Then
                    Echo("-- Visa write message error" & vbCrLf & "  " & InstrumentDescription(InstrumentIndex) & vbCrLf & "  " & MessageToSend)
            ...
            Case Else
                retCount = Len(MessageToSend)
                nSystErr = atxml_WriteCmds(ResourceName(InstrumentIndex), MessageToSend, retCount)
                If (InstrumentIndex = SWITCH1) And (LCase(MessageToSend) = "reset") Then
                    'Note: this is a recursive command, but should not be a problem
...
#End If
    ...
    Public Function ReturnNoCommString(iInstrumentNoComm As Integer, iInstrumentNotTested As Integer) As String
        '******************************************************************************************
        '***    Returns a String that indicates which Instrument could not establish            ***
        '***    communications and which Instrument could not be tested.                        ***
        '***    This is recorded in the Comments field of the FHDB.                             ***
        '***                                                         Dave Joiner 07/10/2001     ***
        '******************************************************************************************
        Dim sFirstString As String      'Holds the First String
        Dim sLastString As String       'Holds the Last String
