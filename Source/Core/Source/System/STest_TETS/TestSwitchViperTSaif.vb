...
Public Module modSwitchingViperT
    ...
    '*                                                             *
    '* Nomenclature   : System Self Test: Switch Test              *
    ...
    Function ClosureDetected(ByVal SwitchLable As String, ByVal dLower As Double, ByVal dUpper As Double) As Boolean
        ...
        ElseIf dMeasurement > (dUpper - CleanTestX) Then
            If (EnableClean = True) And (CleanSwitch <> "") Then
                ...
                temp = "    Clean " + FnFindSwitchLogicName(CleanSwitch) + " Before: " & EngNotate(dMeasurement) + "Ohms"
    ...
    Function OpenDetected(ByVal SwitchNum As String, ByVal sInstrumentCode As String) As Short
        'DESCRIPTION:
        ...
        If Val(StrMeas) < MinImp Or retVal <> 0 Then
            ...
            OpenDetected = FAILED
            FormatResultLine(sTNum & " " & FnFindSwitchLogicName(SwitchNum) & " Open Test Measured: " & EngNotate(dReturnResistance, 3) & "Ohms ", False) 'failed

            'Report Failure to FHDB
            RegisterFailure(CStr(InstrumentToTest), sTNum, dReturnResistance, "Ohms", MinImp, , FnFindSwitchLogicName(SwitchNum))

        Else            'Report PASSED status to the System Log
            OpenDetected = PASSED
            FormatResultLine(sTNum & " " & FnFindSwitchLogicName(SwitchNum) & " Open Test ", True) ' passed
        End If

    End Function


    Function Switch1or2Proc(ByVal InstrumentToTest As Integer) As Integer
        ...
        Switch1or2Proc = -1 ' untested
        Application.DoEvents()
        If AbortTest = True Then GoTo TestComplete
        AbortTest = False
        Switch1or2Proc = 0 ' passed
        ...
        frmSTest.proProgress.Maximum = 300
...
Step2:          ' install W203 MF cable
                S = "Install the W203 cable (93006H6044) as follows:" & vbCrLf
                S &= "1. Connect W203-P1 cable onto the SAIF J2 connector. " & vbCrLf
                S &= "2. Connect W203-P2 cable onto the SAIF front panel MF IN connector via a BNC 50 Ohm Feedthrough Terminator. " & vbCrLf
                S &= "3. Connect W203-P3 cable onto the SAIF front panel MF OUT connector. " & vbCrLf
...
LF1_1_4:    'LF1-01-004        Switch RESET Self test
            ...
            If (SError <> 0) Then
                ...
                Echo("  Reset Test error<" & SError & "> from module" & Str(mModule))
                Echo("  This is a possible DIP switch error on one of the switch cards in the station.")
...
LF1_1_6:  'LF1-01-006; LF2-01-002       Open all switches Test
        ...
        nSystErr = WriteMsg(DMM, "*RST;*CLS") ' init DMM
...
LF1_2_1:
            ...
            If (iStatus <> 0) Or (retVal <> 0) Or (mModule = CInt(ModuleNo)) Then
                sMsg = FnFindSwitchLogicName(Channels(Channel)) & " Open Test"
...
LF1_2_2:
            ...
            PathImpedance = 0
            TestSwitch = FnFindSwitchLogicName(Channels(Channel)) ' S101-1,2
            EnableClean = True
        ...
        If PathImpedance > 10 Then ' invalid path impedance, try again
            ...
            If PathImpedance > 10 Then ' invalid path impedance
                Echo("---- Path Impedance too high at" & Str(PathImpedance) & " Ohms!")
        ...
        For group = 1 To 4
            Select Case group
                Case 1
                    ...
                    TestSwitch = FnFindSwitchLogicName(ModuleNo & ".1028-1032")
                Case 2
...
LF1_2_11:
            ...
            For Channel = 1 To 5 '5 parallel switches per group
                ...
                Select Case group ' 4 groups
                    Case 1
                        ChannelNo = ModuleNo & ".10" & Format(Channel + 27, "00") ' S301-57,58 to 65,66
                    Case 2
                        ChannelNo = ModuleNo & ".10" & Format(Channel + 32, "00") ' S301-67,68 to 75,76
                    Case 3
                        ChannelNo = ModuleNo & ".10" & Format(Channel + 37, "00") ' S301-77,78 to 85,86
                    Case 4
                        ChannelNo = ModuleNo & ".10" & Format(Channel + 42, "00") ' S301-87,88 to 95,96
                End Select
...
LF1_2_12:
                TestSwitch = FnFindSwitchLogicName(ChannelNo) 'S301-57,58
        ...
        For PathX = 1 To 10
            ...
            Echo(vbCrLf & "---- Test " & FnFindSwitchLogicName(TestSwitch))
...
LF1_3_2:
                TestSwitch = FnFindSwitchLogicName(ChannelNo)
            ...
            Select Case PathX
                Case 1
                    ChannelNo = ModuleNo & ".1011-1027" ' S301-23 to 56
                Case 2
                    ChannelNo = ModuleNo & ".2200,2201" ' S603-1,2(3)
                Case 3
                    ChannelNo = ModuleNo & ".2300,2301" ' S604-1,2(3)
                Case 4
                    ChannelNo = ModuleNo & ".2400,2401" ' S605-1,2(3)
                Case 5
                    ChannelNo = ModuleNo & ".2500,2501" ' S606-1,2(3)
                Case 6
                    ChannelNo = ModuleNo & ".4000-4007" ' S501-1 to 3-10 (one half of S501)
                Case 7
                    ChannelNo = ModuleNo & ".4100-4107" ' S501-1 to 3-10
                Case 8
                    ChannelNo = ModuleNo & ".4200-4207" ' S501-1 to 3-10
                Case 9
                    ChannelNo = ModuleNo & ".4300-4307" ' S501-1 to 3-10
                Case 10
                    ChannelNo = ModuleNo & ".4400-4407" 'S501-1 to 3-10
            End Select
            nSystErr = WriteMsg(SWITCH1, "OPEN " & ChannelNo)
...
LF1_3_5:
            ...
            If PathX > 5 Then
                ...
                Select Case PathX
                    Case 6
                        ChannelNo = ModuleNo & ".4010-4017"
                    Case 7
                        ChannelNo = ModuleNo & ".4110-4117"
                    Case 8
                        ChannelNo = ModuleNo & ".4210-4217"
                    Case 9
                        ChannelNo = ModuleNo & ".4310-4317"
                    Case 10
                        ChannelNo = ModuleNo & ".4410-4417"
                End Select
...
LF1_8_1:
                ...
                Select Case PathX
                    Case 6
                        ChannelNo = ModuleNo & ".4000-4007"
                    Case 7
                        ChannelNo = ModuleNo & ".4100-4107"
                    Case 8
                        ChannelNo = ModuleNo & ".4200-4207"
                    Case 9
                        ChannelNo = ModuleNo & ".4300-4307"
                    Case 10
                        ChannelNo = ModuleNo & ".4400-4407"
                End Select
                nSystErr = WriteMsg(SWITCH1, "OPEN " & ChannelNo)
            End If
...
LF1_3_6:
            ...
            Select Case PathX
                Case 1
                    ChannelNo = ModuleNo & ".3200,1011-1027,2000" ' S301 switches
                Case 2
                    ChannelNo = ModuleNo & ".3201,2300-2301,2200-2201,2100,2001" ' S603,S604
                Case 3
                    ChannelNo = ModuleNo & ".3201,2300-2301,2200-2201,2100,2001" ' S603,S604
                Case 4
                    ChannelNo = ModuleNo & ".3202,2500-2501,2400-2401,2101,2001" ' S605,S606
                Case 5
                    ChannelNo = ModuleNo & ".3202,2500-2501,2400-2401,2101,2001" ' S605,S606
                Case 6
                    ChannelNo = ModuleNo & ".3203,4000-4417,2000" ' S501
                Case 7
                    ChannelNo = ModuleNo & ".3203,4000,4100-4117,4011,2000" ' S502
                Case 8
                    ChannelNo = ModuleNo & ".3203,4002,4200-4217,4013,2000" ' S503
                Case 9
                    ChannelNo = ModuleNo & ".3203,4004,4300-4317,4015,2000" ' S504
                Case 10
                    ChannelNo = ModuleNo & ".3203,4006,4400-4417,4017,2000" ' S505
            End Select
        ...
        For PathX = 1 To 4
            ...
            Select Case PathX
                Case 1
                    Channels(1) = ModuleNo & ".3000" : Channels(2) = ModuleNo & ".3100"
                Case 2
                    Channels(1) = ModuleNo & ".3001" : Channels(2) = ModuleNo & ".3101"
                Case 3
                    Channels(1) = ModuleNo & ".3002" : Channels(2) = ModuleNo & ".3102"
                Case 4
                    Channels(1) = ModuleNo & ".3003" : Channels(2) = ModuleNo & ".3103"
            End Select
            ...
            Select Case PathX
                Case 1
                    MaxImp = 8 : PathNo = ModuleNo + ".3000,3100" 'S401-1,2,S402-1,2 or S404-1,2,S406=1,4
                Case 2
                    MaxImp = 8 : PathNo = ModuleNo & ".3001,3101" 'S401-1,3,S402-1,3 or S404-1,3,S405-1,3
                Case 3
                    MaxImp = 9 : PathNo = ModuleNo & ".3002,3102,1000-1010" 'S401-1,4,S402-1,4,S301-1-22 or S404-1,4,S405-1,4,S301-97-118
                Case 4
                    MaxImp = 8 : PathNo = ModuleNo & ".3003,3103" 'S401-1,5,S402-1,5 or S404-1,5,S406=1,5
            End Select
            nSystErr = WriteMsg(SWITCH1, "CLOSE " & PathNo) ' closes the whole path
            Echo(vbCrLf & "---- Test " & FnFindSwitchLogicName(PathNo))
...
TestComplete:
        ...
        nSystErr = WriteMsg(SWITCH1, "RESET") ' open all switches, then close interconnect relays
        ...
        nSystErr = WriteMsg(DMM, "*RST;*CLS") ' init DMM
        If Switch1or2Proc = 1 Then 'If Test Fails, register failure
        ...
        If AbortTest = True Then
            ...
            sMsg += "      *      LF" & ModuleNo & " and DMM have been reset.          *" & vbCrLf
            sMsg += "      **********************************************" & vbCrLf
            Echo(sMsg)
            ''sMsg = "LF" & ModuleNo & " Switch test aborted!" & vbCrLf
            ''sMsg += "LF" & ModuleNo & " and DMM have been reset." & vbCrLf
...
Step2:          ' install W203 MF cable
                S = "Install the W203 cable (93006H6044) as follws:" & vbCrLf
                S &= "1. Connect W203-P1 cable onto the SAIF J2 connector. " & vbCrLf
                S &= "2. Connect W203-P2 cable onto the SAIF front panel MF IN connector via a BNC 50 Ohm Feedthrough Terminator. " & vbCrLf
                S &= "3. Connect W203-P3 cable onto the SAIF front panel MF OUT connector. " & vbCrLf
                DisplaySetup(S, "ST-W203-1.jpg", 3, True, 2, 2)
...
LF3_1_1: sTNum = "LF3-01-001"
        ...
        If (SError <> 0) And (mModule = 3) Then
            ...
            Echo("  Close BIT error<" & SError & "> from module" & Str(mModule))
            Echo("  This is a possible DIP switch error on one of the switch cards in the station.")
            Echo("  If possible, try swapping switch cards to identify the bad card, and ")
            Echo("    then try toggling each DIP switch on that card.")
...
LF3_1_2: sTNum = "LF3-01-002"
        ...
        If SError <> 0 Then
            ...
            Echo("  Open BIT error<" & SError & "> from module" & Str(mModule))
            Echo("  This is a possible DIP switch error on one of the switch cards in the station.")
            Echo("  If possible, try swapping switch cards to identify the bad card, and ")
            Echo("    then try toggling each DIP switch on that card.")
        ...
        For Channel = 0 To 7
            ...
            TestSwitch = FnFindSwitchLogicName(sS751)
            CleanSwitch = sS751
...
TestComplete:
        ...
        If AbortTest = True Then
            ...
            sMsg += "      *          LF3 and DMM have been reset.      *" & vbCrLf
            sMsg += "      **********************************************" & vbCrLf
            Echo(sMsg)
            ''sMsg = "LF3 Switch test aborted!" & vbCrLf & vbCrLf
            ''sMsg += "LF3 and DMM have been reset." & vbCrLf
            ''   MsgBox(sMsg)
        End If
    ...
    Function Switch4Proc() As Integer
        ...
        If (RunningEndToEnd = False) And (FirstPass = True) Then
            X = MsgBox("Are the SP1 adapter and the W203 cable installed?", MsgBoxStyle.YesNo)
            If X <> DialogResult.Yes Then
Step1:          ' install adapter SP1
                S = "Install the SP1 Adapter (93006H6042) onto the SAIF J1 connector. " & vbCrLf
                DisplaySetup(S, "ST-SP1-1.jpg", 1, True, 1, 2)
...
MF_1_1:
            ...
            If (SError <> 0) Then
                ...
                Echo("  Close Test error<" & SError & "> from module" & Str(mModule))
                Echo("  This is a possible DIP switch error on one of the switch cards in the station.")
                Echo("  If possible, try swapping switch cards to identify the bad card, and ")
                Echo("    then try toggling each DIP switch on that card.")
...
MF_1_2:
        ...
        For PairNo = 0 To 1
            ...
            For Channel = 0 To 7
                ...
                TestSwitch = FnFindSwitchLogicName(ChannelNo1) & " and " & FnFindSwitchLogicName(ChannelNo2)
...
MF_2_1:
                ...
                iSubStep += 1
                LoopStepNo += 1
                TestSwitch = FnFindSwitchLogicName(ChannelNo1)
...
MF_2_2:
                ...
                iSubStep += 1
                LoopStepNo += 1
                TestSwitch = FnFindSwitchLogicName(ChannelNo2)
                nSystErr = WriteMsg(SWITCH1, "CLOSE " & ChannelNo1)
...
TestComplete:
        ...
        nSystErr = WriteMsg(SWITCH1, "CLOSE 3.1001,2002,3000,3001,4000,4001,6000,6001,7000,7001")
        nSystErr = WriteMsg(DMM, "*RST;*CLS") ' init DMM
        If Switch4Proc = 1 Then 'If Test Fails, register failure
        ...
        If AbortTest = True Then
            ...
            sMsg += "      *           MF and DMM have been reset.      *" & vbCrLf
            sMsg += "      **********************************************" & vbCrLf
            Echo(sMsg)
            sMsg = "MF Switch test aborted!" & vbCrLf
            sMsg += "MF and DMM have been reset." & vbCrLf
...
Step2:
            ...
            S += "2. Connect SAIF J3-1 to SAIF front panel HF IN via a W207 (SMA to N) cable. " & vbCrLf
            S += "3. Connect SAIF J3-2 to SAIF J4-2 via a W202 (#1) cable. " & vbCrLf
            S += "4. Connect SAIF J3-3 to SAIF J4-3 via a W202 (#1) cable. " & vbCrLf
            S += "5. Connect SAIF J3-4 to SAIF J4-4 via a W202 (#1) cable. " & vbCrLf
            S += "6. Connect SAIF J3-5 to SAIF J4-5 via a W202 (#2) cable. " & vbCrLf
            S += "7. Connect SAIF J3-6 to SAIF J4-6 via a W202 (#2) cable. " & vbCrLf
            S += "8. Connect SAIF J3-7 to SAIF J4-7 via a W202 (#2) cable. " & vbCrLf
            S += "9. Connect SAIF J4-1 to 50 Ohm terminator via SMA to N adapter. " & vbCrLf
            S += "10. Connect SAIF DMM-INPUT HI/LO to SAIF front panel HF OUT via a W21 (Dual Banana Jack to N) cable." & vbCrLf
...
HF_1_2:
        ...
        For PairNo = 0 To 2 ' three pairs of switches
            ...
            For Channel = 0 To 5
                ...
                TestSwitch = FnFindSwitchLogicName(ChannelNo1) & ", " & FnFindSwitchLogicName(ChannelNo2)
...
TestComplete:
        ...
        nSystErr = WriteMsg(SWITCH1, "CLOSE 3.1001,2002,3000,3001,4000,4001,6000,6001,7000,7001")
        nSystErr = WriteMsg(DMM, "*RST;*CLS") ' init DMM
        If Switch5Proc = 1 Then 'If Test Fails, register failure
            RegisterFailure(SWITCH5, , , , , , "HF Switch Module Failed")
        End If
        If AbortTest = True Then
            sMsg = vbCrLf + "      **********************************************" & vbCrLf
            sMsg += "      *           HF Switch test aborted!          *" & vbCrLf
            sMsg += "      *      HF switch and DMM have been reset.    *" & vbCrLf
            sMsg += "      **********************************************" & vbCrLf
            Echo(sMsg)
            sMsg = "HF Switch test aborted!" & vbCrLf
            sMsg += "HF Switch and DMM have been reset." & vbCrLf
    ...
    Function TestSwitchingViperT(ByVal InstrumentToTest As Integer) As Integer
        'DESCRIPTION
        ...
        If FailOccur Then ' failoccur 0=passed, 1=failed?
    ...
    Public Function FnFindSwitchLogicName(ByVal ChannelString As String) As String
        FnFindSwitchLogicName = ""
        'DESCRIPTION:
        ...
        ChannelNumber(1) = Mid(ChannelNumber(1), 3)
        ...
        End If
        FnFindSwitchLogicName = LocalString
    ...
    Function FnNameSwitch(ByVal ModuleX As Integer, ByVal Designator As Integer) As String
        ...
        If ModuleX = 1 Then ' LF1 1.0000-1.4417
            Select Case Designator
                ...
                Case 1
                    NameX = "S101-5,6" '(7,8)"
                Case 2
                    NameX = "S101-9,10" '(11,12)"
                Case 3
                    NameX = "S101-13,14" '(15,16)"
                Case 4
                    NameX = "S101-17,18" '(19,20)"
                Case 1000 To 1047
        ...
        ElseIf ModuleX = 2 Then            ' LF2 2.0000-2.4417
            Select Case Designator
                Case 0
                    NameX = "S101-21,22" '(23,24)"
                Case 1
                    NameX = "S101-25,26" '(27,28)"
                Case 2
                    NameX = "S101-29,30" '(31,32)"
                Case 3
                    NameX = "S101-33,34" '(35,36)"
                Case 4
                    NameX = "S101-37,38" '(39,40)"
                Case 1000 To 1047
