        ...
        If FirstPass = True Then
            ...
            If x <> DialogResult.Yes Then
                S = "1. Remove all cables/adapters from the SAIF." & vbCrLf
                    S &= "2. Connect cable W201-P1 (93006H6045) to SAIF J1 connector." & vbCrLf
                    S &= "3. Note: W201-P2 to P23 BNC cables maybe left on or disconnected." & vbCrLf
                    DisplaySetup(S, "ST-W201-1.jpg", 3)
                If AbortTest = True Then GoTo TestComplete
            End If
            ElseIf (ControlStartTest = PPU) Then

Step1:          'Install cable W201
                InstallW201Cable()
...
PPU_2:      'P01-02-001 to P10-02-001 Open Relay Test
            ...
            dMeasurement = FetchMeasurement(DMM, "PPU Supply DC" & CStr(Supply) & " Open Relay Test", "", "1", "V", PPU, sTNum)
...
PPU_3:      'P01-03-001 to P10-03-001 No Load Voltage Test
            ...
            d1Measurement = FetchMeasurement(DMM, "PPU Supply DC" & CStr(Supply) & " No Load Voltage Test", CStr(VoltsHi - 1), CStr(VoltsHi + 1), "V", PPU, sTNum)
...
PPU_4:      'P01-04-001 to P10-04-001 'Local Sense Voltage Test under Load
            ...
            'Connect 13 ohm UTILITY LOADS (HI/LO) to DC POWER SUPPLIES DC9 or DC10.
            nSystErr = WriteMsg(SWITCH1, "CLOSE 1.0000") 'connect DC1 to DC2
            If Supply < 10 Then
            ...
            d1Measurement = FetchMeasurement(DMM, "PPU Supply DC" & CStr(Supply) & " Local Sense Voltage Test", "11", "13", "V", PPU, sTNum)
...
PPU_5:      'P01-05-001 to P10-05-001 Remote sense Voltage Test
            ...
            d2Measurement = FetchMeasurement(DMM, "PPU Supply DC" & CStr(Supply) & " Remote Sense Voltage Test", "11", "13", "V", PPU, sTNum)
...
PPU_8:      'P01-08-001 to P10-08-001  Voltage Ripple Test max 80mVac
            ...
            dMeasurement = FetchMeasurement(DMM, "PPU Supply DC" & CStr(Supply) & " Voltage Ripple Test", "", "0.08", "VAC", PPU, sTNum)
...
PPU_9:      'P01-09-001 to P10-09-001  Negative Voltage Test under load
            ...
            dMeasurement = FetchMeasurement(DMM, "PPU Supply DC" & CStr(Supply) & " Negative Voltage Test", "-13", "-11", "V", PPU, sTNum)
...
PPU_10:     'P01-10-001 to P10-10-001  Overload Trip Test
            ...
            nSystErr = WriteMsg(DMM, "MEAS:VOLT?")
            Delay(0.1)
            d1Measurement = FetchMeasurement(DMM, "PPU Supply DC" & CStr(Supply) & " Excess Current Shut Down Test", "0", "2", "V", PPU, sTNum)
...
TestComplete:
   ...
   Function CommandCalDateRead(ByVal Supply As Integer) As Integer
    ...
    Function CommandMeasureAmps(ByVal Supply As Integer) As Single
        ...
        B1 = Supply
        B2 = &H42
        B3 = 0 ' changed to 0 for Viper/T, was 128 for the TETS bb
    ...
    Sub SendDCPSCommand(ByVal slot As Integer, ByVal Command As String)
        ...
        If ErrorStatus <> 0 Then
            '  Echo "Power Supply #" & Cstr(slot) & " Error#" & Cstr(ErrorStatus&)) 'bb temp
        End If
...
#If defUse_CommandCalDateWrite Then
    Function CommandCalDateWrite(ByVal Supply As integer) As Single
        ...
        'century
        SendDCPSCommand(Supply, Chr(&H80+Supply) & Chr(&H4F) & Chr(&HAA))
        SendDCPSCommand(Supply, Chr(&H80+Supply) & Chr(&H60+i) & Chr(Wcentury))

        'year
        SendDCPSCommand(Supply, Chr(&H80+Supply) & Chr(&H4F) & Chr(&HAB))
        SendDCPSCommand(Supply, Chr(&H80+Supply) & Chr(&H60+i) & Chr(Wyear))

        'month
        SendDCPSCommand(Supply, Chr(&H80+Supply) & Chr(&H4F) & Chr(&HAC))
        SendDCPSCommand(Supply, Chr(&H80+Supply) & Chr(&H60+i) & Chr(Wmonth))

        'day
        SendDCPSCommand(Supply, Chr(&H80+Supply) & Chr(&H4F) & Chr(&HAD))
        SendDCPSCommand(Supply, Chr(&H80+Supply) & Chr(&H60+i) & Chr(Wday))
        Echo("Write Supply "+Cstr(Supply)+vbTab+">"+Format(Wcentury, "00")+Format(Wyear, "00")+"-"+Format(Wmonth, "00")+"-"+Format(Wday, "00"))
