...
Module TETS
    ...
    '*     button have the TETS instrumentation Reset and return*
    '*     to the Main Menu                                     *
    '*    EXAMPLE:                                              *
    '*     TETS.ResetSystem                                     *
    '*    PARAMETERS:                                           *
    ...
    Declare Function eip1315a_init Lib "rfcnt.dll" (ByVal ResourceName As String, instrumentHandle As Long) As Long
    ''Declare Function eip1315a_setLO Lib "rfcnt.dll" (ByVal theSession As Long, ByVal Frequency As Double, ByVal mode, ByVal attn As Long, ByVal IFDET) As Long
    ''Declare Function eip1315a_close Lib "rfcnt.dll" (ByVal instrumentHandle As Long) As Long
    ...
    Declare Function rfcnt_init Lib "rfcnt.dll" (ByVal ResourceName As String, instrumentHandle As Long) As Long
    Declare Function gt55210a_init Lib "rfcnt.dll" (ByVal ResourceName As String, instrumentHandle As Long) As Long
    Declare Function rfcnt_reset Lib "rfcnt.dll" (ByVal instrSession As Long) As Long
    Declare Function rfcnt_writeInstrData Lib "rfcnt.dll" (ByVal instrSession As Long, ByVal writeBuffer As String) As Long
    Declare Function rf_measfreq Lib "rfcnt.dll" (ByVal ihDC As Long, ByVal ihCNT As Long, ByVal expectedfreq As Double, ByVal attn As Long, measfreq As Double) As Long
    Declare Function rf_measpw Lib "rfcnt.dll" (ByVal ihDC As Long, ByVal ihCNT As Long, ByVal carrierfreq As Double, ByVal expectedpw As Double, ByVal attn As Long, measpw As Double) As Long
    Declare Function rf_measpd Lib "rfcnt.dll" (ByVal ihDC As Long, ByVal ihCNT As Long, ByVal carrierfreq As Double, ByVal expectedpw As Double, ByVal attn As Long, measpd As Double) As Long
    ''end add
    ''
    ''5/8/03 by Soon Nam
    ''added for RF Stim dll calls
    ''Declare Function gt50000_selectCorrectionsTable Lib "gt50000.dll" (ByVal FilePath As String, ByVal TableName As String) As Long
    ''Declare Function gt50000_setCorrectedPower Lib "gt50000.dll" (ByVal instrSession As Long, ByVal Frequency As Double, ByVal Power As Single) As Long
    Declare Function selectCorrectionsTable Lib "gt50000.dll" Alias "gt50000_selectCorrectionsTable" (ByVal FilePath As String, ByVal TableName As String) As Long
    Declare Function setCorrectedPower Lib "gt50000.dll" Alias "gt50000_setCorrectedPower" (ByVal instrSession As Long, ByVal Frequency As Double, ByVal Power As Single) As Long
    ''end add
    ''
    ''6/24/03 Soon Nam
...
'    Declare Function terM9_close Lib "terM9_32.dll" (ByVal vi As Integer) As Integer
    ...
    Public Const LAST_CORE_INST As Short = SWITCH4
    Public Const RFPM As Short = 15
    Public Const RFCOUNTER As Short = 16
    Public Const RFREC As Short = 17
    Public Const MOD_ANAL As Short = 18
    Public Const RFSYN As Short = 19
    Public Const LAST_RF_INST As Short = RFSYN
    Public Const EO_VC As Short = 20
    Public Const EO_MF As Short = 21
    Public Const EO_IR As Short = 22
    Public Const EO_VIS As Short = 23
    Public Const EO_MS As Short = 24
    Public Const SYN_RES As Short = 25
    Public Const RFMEAS As Short = 26
    Public Const RS422 As Short = 27
    Public Const RS232 As Short = 28
    Public Const GIGA As Short = 29
    Public Const LAST_EO_INST As Short = GIGA
    ...
    ' It is initialized to False by 'iTETSinit'.
    ...
    Public sInstFileName(LAST_INSTRUMENT) As String
    ...
    Public sInstrumentDescription(LAST_INSTRUMENT) As String
    ...
    Sub ResetSystem()
        ...
        If bInstrRequired(RFPM) Then SystErr = atxmlDF_viClear(ResourceName(RFPM), 0)
        ...
        If bInstrRequired(RFCOUNTER) Then SystErr = atxmlDF_viClear(ResourceName(RFCOUNTER), 0)
        If bInstrRequired(MOD_ANAL) Then SystErr = atxmlDF_viClear(ResourceName(MOD_ANAL), 0)
        ...
        If bInstrRequired(DIGITAL) Then
            nInitStatus = terM9_init("TERM9::#0", 1, 1, nInstrumentHandle(DIGITAL))
            SystErr = terM9_reset(nInstrumentHandle(DIGITAL))
    ...
    Function iTETSInit() As Short
        ...
        If Not IO.File.Exists(VisaLibrary) Then
            ...
            bLiveMode = False
            nSessionHandle = 0
            Echo("SYSTEM ERROR - Unable to initialize TETS System. Can't find VISA32.DLL.")
            Exit Function
        Else
        ...
        If lSystErr < 0 Then
            ...
            bLiveMode = False
            nSessionHandle = 0
            Echo("SYSTEM ERROR - Unable to initialize TETS System. Can't obtain VISA session handle.")
            Exit Function
        Else
            gFrmMain.txtMeasured.Text = "TRUE"
        End If
        ...
        If bRFOptionInstalled Then
            ...
            If Not bCheckOptionInstalled("RFSTIM") Then
                sIDNResponse(RFSYN) = "1143A"
                sInstrumentDescription(RFSYN) = "EIP 1143A RF Stimulus"
            End If
        Else
            gFrmMain.txtMeasured.Text = "FALSE"
        End If
        ...
        If Not VXIPowerCheck() Then
            ...
            bLiveMode = False
            nSessionHandle = 0
            Echo("SYSTEM ERROR - Unable to initialize TETS System. VXI Power off.")
            Exit Function
        Else
            gFrmMain.txtMeasured.Text = "TRUE"
        End If
        ...
        For iInstrument = 1 To iLastInstrument
            If bInstrRequired(iInstrument) Then
                ...
                Select Case iInstrument
                    Case DMM, COUNTER, ARB, FGEN, OSCOPE, RFPM, RFCOUNTER, RFSYN
    ...
    Function InitMessageBasedInstrument(ByRef InstrumentToInit As Integer) As Short
        Dim ReadBuffer As String = Space(2048)
...
' ***********************
'  Will need to be updated when the wrapper is complete
...
' ***********************
        ...
        If IDSpec <> "50000" Then
            Dummy = atxmlDF_viClear(iHandle, 0)
        End If
    ...
    Sub TETSVar()
        ...
        sIDNResponse(RFSYN) = "50000"
        sIDNResponse(OSCOPE) = "E1428A"
        ...
        sInstFileName(RFCOUNTER) = "RFC"
        sInstFileName(MOD_ANAL) = "RFM"
        sInstFileName(RFSYN) = "RFS"
        sInstFileName(RFPM) = "RFP"
        sInstFileName(RFREC) = "RFD"
        sInstFileName(OSCOPE) = "DSCOPE"
        sInstFileName(PPU) = "UUTPS"
        sInstFileName(MIL_STD_1553) = "MIL1553"
        sInstFileName(EO_VC) = "EOVC"
        sInstFileName(EO_MF) = "EOMF"
        sInstFileName(EO_IR) = "EOIR"
        sInstFileName(EO_VIS) = "EOVIS"
        sInstFileName(EO_MS) = "EOMS"
        ...
        sInstrumentIDCode(MOD_ANAL) = "RMA"
        sInstrumentIDCode(RFPM) = "RPM"
        sInstrumentIDCode(RFREC) = "RDC"
        sInstrumentIDCode(EO_VC) = "EVC"
        sInstrumentIDCode(EO_IR) = "EIR"
        sInstrumentIDCode(EO_VIS) = "EVS"
        sInstrumentIDCode(EO_MF) = "EMF"
        sInstrumentIDCode(EO_MS) = "EMS"
    End Sub
    ...
    ''' </summary>
    ''' <param name="instrumentIndex">the index of the instrument from which to read</param>
    ...
    ''' <remarks></remarks>
    Function ReadMsg(ByRef instrumentIndex As Integer, ByRef ReturnMessage As String) As Integer
        Dim ihandle, Dummy, lRetCount As Integer
        Dim sReadBuffer As String = Space(256)
        ...
        Select Case lSystErr
            ...
            Case Else
                ...
                MsgBox("Error Message:" & vbCrLf & StripNullCharacters(lpBuffer), MsgBoxStyle.Critical, "VISA Error Message")
        End Select
    ...
    ''' </summary>
    ''' <param name="instrumentIndex">the handle to the instrument to write</param>
    ''' <param name="MessageToSend">the string of data to be written</param>
    ''' <remarks></remarks>
    Sub WriteMsg(ByVal instrumentIndex As Integer, ByRef MessageToSend As String)
        Dim errorMessage As String = ""
        Dim ihandle, lRetCount As Integer
        ...
        lSystErr = atxml_WriteCmds(ResourceName(instrumentIndex), MessageToSend, lRetCount)
        ...
        If lSystErr Then
            ...
            MsgBox("Error sending:" & vbCrLf & MessageToSend & vbCrLf & "Error:" & errorMessage & vbCrLf & "See Log File for additional information.", MsgBoxStyle.Exclamation, "WRITEMSG Function Error")
            iWriteMsgError = True
        Else
            Select Case instrumentIndex
                Case DMM, COUNTER, OSCOPE, ARB, FGEN, RFSYN, RFCOUNTER
                    Do
                        System.Windows.Forms.Application.DoEvents()
                        If InStr(MessageToSend, "?") <> 0 Then Exit Do ' If expecting a response
                        If InStr(MessageToSend, "*CLS") <> 0 Then Exit Do
                        If InStr(MessageToSend, "*RST") <> 0 Then Exit Do
                        If instrumentIndex = OSCOPE And Left(MessageToSend, 3) = "DIG" Then Exit Do
                        If instrumentIndex = OSCOPE And Left(MessageToSend, 3) = "CAL" Then Exit Do
                        If instrumentIndex = FGEN And Left(MessageToSend, 5) = "APPLY" Then Delay(2)
                        lSystErr = viWrite(ihandle, "SYST:ERR?", 9, lRetCount)
                        lSystErr = ReadMsg(instrumentIndex, errorMessage)
                        If Val(errorMessage) = 0 Then Exit Do
                        Echo("WRITEMSG ERROR: Error sending command to " & sInstrumentDescription(instrumentIndex) & ".")
                    ...
                    Loop
                Case MOD_ANAL
                    If (InStr(MessageToSend, "T") <> 0 And (InStr(MessageToSend, "WT") = 0) And (InStr(MessageToSend, "TR") = 0)) Or InStr(MessageToSend, "ID") Then
                        'Ignore these: "TS", "TV", "TL", "TP", "TF", "ID"
                    Else
                        Delay(0.5)
                        lSystErr = viWrite(ihandle, "TS", 2, lRetCount)
                        lSystErr = ReadMsg(instrumentIndex, errorMessage)
                        If Val(errorMessage) <> 0 Then
                            iWriteMsgError = Val(errorMessage)
                            Echo("WRITEMSG ERROR: Error sending command to " & sInstrumentDescription(instrumentIndex) & ".")
                            Echo("COMMAND: " & MessageToSend)
                            MsgBox("Error sending:" & vbCrLf & MessageToSend & vbCrLf & "Error:" & errorMessage & vbCrLf & "See Log File for additional information.", MsgBoxStyle.Exclamation, "WRITEMSG Function Error")
                            'Stop
                            lSystErr = viWrite(ihandle, "CL", 2, lRetCount)
                        End If
                    End If
            End Select
        End If
    ...
    Function ReadSW() As String
        ...
        Do
            ErrorStatus = viRead(nInstrumentHandle(TETS.SWITCH1), ReadBuffer, 255, CharsRead)
    ...
    Public Function DecodeErrorMessage(ByRef Msg As String) As String
